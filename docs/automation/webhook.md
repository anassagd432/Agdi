...
read_when:
  ...
  - Wiring external systems into AGDI
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your AGDI config directory (typically `~/.agdi/hooks/transforms`).
...
- `agdi webhooks gmail setup` writes `hooks.gmail` config for `agdi webhooks gmail run`.
