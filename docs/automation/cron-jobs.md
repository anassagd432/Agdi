...
- For upgrades, `agdi doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
To reduce top-of-hour load spikes across many gateways, Agdi applies a
...
- CLI equivalents: `agdi cron add --light-context ...` and `agdi cron edit --light-context`.
...
- `schedule.at` accepts ISO 8601. Tool/API values without a timezone are treated as UTC; the CLI also accepts `agdi cron add|edit --at "<offset-less-iso>" --tz <iana>` for local wall-clock one-shots.
...
- Run history: `~/.agdi/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Agdi classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Agdi archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `agdi cron runs` and adjust retention before logs become large
