...
- `config: true` enables `/config` (reads/writes `agdi.json`).
...
- Override agent config root with `AGDI_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `agdi onboard --auth-choice opencode-zen`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `AGDI_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or AGDI_SKIP_CANVAS_HOST=1
  },
}
```
...
- Hostname defaults to `agdi`. Override with `AGDI_MDNS_HOSTNAME`.
