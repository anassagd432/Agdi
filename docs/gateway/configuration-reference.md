...
summary: "Complete reference for every Agdi config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.agdi/agdi.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Agdi uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.agdi/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `agdi doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `agdi doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["agdi-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-agdi",
          requireMention: false,
...
- Agdi additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `agdi plugins install @openclaw/mattermost`.
...
- `commands.callbackUrl` must resolve to the Agdi gateway endpoint and be reachable from the Mattermost server.
...
Agdi spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `agdi channels add` (or channel onboarding) while still on a single-account top-level channel config, Agdi moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `agdi doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@agdi", "agdi"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@agdi"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `agdi.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Agdi auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/agdi" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Agdi assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Agdi aborts it. Default: `900`.
    ...
    defaults: {
      sandbox: {
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/agdi-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "agdi-sandbox-browser:bookworm-slim",
          network: "agdi-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Agdi materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.agdi/sandboxes`
...
In `remote` mode, host-local edits made outside Agdi are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `agdi.json`.
noVNC observer access uses VNC auth by default and Agdi emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `agdi-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.agdi/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@agdi"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.agdi/workspace-home" },
      { id: "work", workspace: "~/.agdi/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Agdi resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.agdi/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.agdi/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.agdi/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.agdi/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Agdi starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.agdi/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Agdi treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:agdi`       | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.agdi/attachments/<uuid>/` with a `.manifest.json`.
...
Agdi uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.agdi/agents/<agentId>/agent/models.json`.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Agdi forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `agdi onboard --auth-choice opencode-zen` or `agdi onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `agdi onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `agdi onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `agdi onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `agdi onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `agdi onboard --auth-choice minimax-api`.
...
- Loaded from `~/.agdi/extensions`, `<workspace>/.agdi/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Agdi plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Agdi plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Agdi applies those as sanitized agent settings, not as raw Agdi config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `agdi plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      agdi: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or AGDI_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `AGDI_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `AGDI_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `AGDI_APNS_RELAY_BASE_URL` / `AGDI_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `AGDI_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
AGDI_CONFIG_PATH=~/.agdi/a.json \
AGDI_STATE_DIR=~/.agdi-a \
agdi gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.agdi-dev` + port `19001`), `--profile <name>` (uses `~/.agdi-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.agdi/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-agdi-token: <token>`.
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__agdi__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__agdi__/a2ui/`
...
- Also serves A2UI at `/__agdi__/a2ui/`.
...
- Hostname defaults to `agdi`. Override with `OPENCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.agdi/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `agdi dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.agdi/.env` (neither overrides existing vars).
...
- `secrets apply` targets supported `agdi.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.agdi/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/agdi-vault-resolver",
...
- Legacy OAuth imports from `~/.agdi/credentials/oauth.json`.
...
- Default log file: `/tmp/agdi/agdi-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Agdi.`).
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `agdi doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `agdi.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
