...
- `agdi gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `agdi doctor` reports no blocking config/service issues.
- `agdi channels status --probe` shows connected/ready channels.
...
agdi logs --follow
agdi models status
agdi config get agents.defaults.models
```
...
agdi status
agdi channels status --probe
agdi pairing list --channel <channel> [--account <id>]
agdi config get channels
agdi logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `agdi config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `agdi devices list` then `agdi devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `agdi configure`). If you are running Agdi via Podman using the dedicated `agdi` user, the config lives at `~agdi/.agdi/agdi.json`.
...
agdi approvals get --node <idOrNameOrIp>
agdi logs --follow
agdi status
```
...
```bash
agdi browser status
agdi browser start --browser-profile agdi
...
```bash
agdi gateway status
agdi config get gateway.mode
agdi config get gateway.remote.url
agdi config get gateway.auth.mode
```
