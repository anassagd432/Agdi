...
Agdi supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `AGDI_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Agdi runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Agdi validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Agdi QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/agdi"],
...
- Agdi resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `agdi.json` refs)
...
When reload-time activation fails after a healthy state, Agdi enters degraded secrets state.
...
- Strict command paths (for example `agdi memory` remote-memory paths and `agdi qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `agdi status`, `agdi status --all`, `agdi channels status`, `agdi channels resolve`, `agdi security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `agdi secrets reload`.
...
```bash
agdi secrets audit --check
agdi secrets configure
agdi secrets audit --check
```
...
- plaintext values at rest (`agdi.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `agdi.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `agdi secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `agdi.json` plus `auth-profiles.json` for one agent scope
...
- `agdi secrets configure --providers-only`
- `agdi secrets configure --skip-provider-setup`
- `agdi secrets configure --agent <id>`
...
```bash
agdi secrets apply --from /tmp/agdi-secrets-plan.json
agdi secrets apply --from /tmp/agdi-secrets-plan.json --allow-exec
agdi secrets apply --from /tmp/agdi-secrets-plan.json --dry-run
agdi secrets apply --from /tmp/agdi-secrets-plan.json --dry-run --allow-exec
```
...
Agdi intentionally does not write rollback backups containing historical plaintext secret values.
