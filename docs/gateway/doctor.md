...
`agdi doctor` is the repair + migration tool for Agdi. It fixes stale
...
```bash
agdi doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
agdi doctor --non-interactive
```
...
- Extra workspace dir detection (`~/agdi`).
...
you to run `agdi doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.agdi/agdi.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.agdi/sessions/` to `~/.agdi/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.agdi/agent/` to `~/.agdi/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.agdi/credentials/*.json` (except `oauth.json`)
  - to `~/.agdi/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `agdi doctor`.

### 3b) Legacy cron store migrations

Doctor also checks the cron job store (`~/.agdi/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.agdi` folders exist across
  home directories or when `AGDI_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.agdi/agdi.json` is
...
offers to remove them and install the Agdi service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Agdi gateway services are considered first-class and are not
flagged as "extra."
...
- `agdi doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `agdi doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `agdi doctor` prompts before rewriting supervisor config.
- `agdi doctor --yes` accepts the default repair prompts.
- `agdi doctor --repair` applies recommended fixes without prompts.
- `agdi doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `agdi gateway install --force`.
