...
- During heartbeat runs, Agdi treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `agdi sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Agdi skips the heartbeat run entirely (no model call).
...
```bash
agdi system event --text "Check for urgent follow-ups" --mode now
```
