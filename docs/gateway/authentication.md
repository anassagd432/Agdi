...
Agdi supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `agdi gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.agdi/.env` so the daemon can read it:
...
API keys for daemon use: `agdi onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.agdi/.env`, systemd/launchd).
...
```bash
agdi models auth paste-token --provider anthropic
agdi models auth paste-token --provider openrouter
```
...
- Agdi retries with the next key only for rate-limit errors (for example
...
Run `agdi models status` to confirm which profile is expiring. If the profile
