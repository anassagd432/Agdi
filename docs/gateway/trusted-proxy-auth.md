...
read_when:
  - Running Agdi behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Agdi
...
- You run Agdi behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Agdi checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Agdi extracts the user identity from the configured header
...
- Agdi can stay on loopback HTTP behind the proxy.
...
If Agdi itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://agdi.example.com
    to: http://agdi-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`agdi security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Agdi config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `agdi security audit` and review findings
