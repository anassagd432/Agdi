...
- Tighten local perms (`~/.agdi` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).
...
- **Pairing allowlists**: `~/.agdi/credentials/<channel>-allowFrom.json`
...
- If you install plugins from npm (`agdi plugins install <npm-spec>`), treat it like running untrusted code:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.agdi/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
4. **Re-run audit**
   - `agdi security audit --deep` and confirm the report is clean.
...
4. **Environment variable** (alternative): set `AGDI_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
authentication. AGDI verifies the identity by resolving the
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only AGDI clients can connect.
