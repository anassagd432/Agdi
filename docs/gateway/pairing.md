...
agdi nodes reject <requestId>
agdi nodes status
agdi nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.agdi`):
