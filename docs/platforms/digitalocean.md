---
summary: "AGDI on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up AGDI on DigitalOcean
  - Looking for cheap VPS hosting for AGDI
title: "DigitalOcean"
---

# AGDI on DigitalOcean

## Goal

Run a persistent AGDI Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status agdi-gateway.service

# View logs
journalctl --user -u agdi-gateway.service -f
```
...
```bash
agdi config set gateway.bind tailnet
agdi gateway restart
```
...
- `~/.agdi/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u agdi --no-pager -n 50
```
