---
summary: "AGDI on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up AGDI on Oracle Cloud
  - Looking for low-cost VPS hosting for AGDI
  - Want 24/7 AGDI on a small server
title: "Oracle Cloud"
---

# AGDI on Oracle Cloud (OCI)

## Goal

Run a persistent AGDI Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for AGDI (especially if you already have an OCI account), but it comes with tradeoffs:
...
This enables Tailscale SSH, so you can connect via `ssh agdi` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@agdi` (or use the Tailscale IP).
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `agdi security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.agdi`
- **Security audit:** `agdi security audit`
...
# Re-authenticate
sudo tailscale up --ssh --hostname=agdi --reset
```
...
journalctl --user -u agdi-gateway -n 50
```
...
- `~/.agdi/` — config, credentials, session data
- `~/.agdi/workspace/` — workspace (SOUL.md, memory, artifacts)

Back up periodically:

```bash
tar -czvf agdi-backup.tar.gz ~/.agdi ~/.agdi/workspace
```
