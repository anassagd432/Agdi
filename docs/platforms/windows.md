...
Agdi supports both **native Windows** and **WSL2**. WSL2 is the more
...
- local CLI use such as `agdi --version`, `agdi doctor`, and `agdi plugins list --json`
- embedded local-agent/provider smoke such as:

```powershell
agdi agent --local --agent main --thinking low -m "Reply with exactly WINDOWS-HATCH-OK."
```

Current caveats:

- `agdi onboard --non-interactive` still expects a reachable local gateway unless you pass `--skip-health`
- `agdi onboard --non-interactive --install-daemon` and `agdi gateway install` try Windows Scheduled Tasks first
- if Scheduled Task creation is denied, Agdi falls back to a per-user Startup-folder login item and starts the gateway immediately
- if `schtasks` itself wedges or stops responding, Agdi now aborts that path quickly and falls back instead of hanging forever
- Scheduled Tasks are still preferred when available because they provide better supervisor status

If you want the native CLI only, without gateway service install, use one of these:

```powershell
agdi onboard --non-interactive --skip-health
agdi gateway run
```
...
### 2) Install the Agdi gateway user service
...
```bash
systemctl --user is-enabled agdi-gateway
systemctl --user status agdi-gateway --no-pager
```
