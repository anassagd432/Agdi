...
- Same path: run the app from a fixed location (for Agdi, `dist/Agdi.app`).
...
```bash
sudo tccutil reset Accessibility ai.agdi.mac
sudo tccutil reset ScreenCapture ai.agdi.mac
...
Workaround: move files into the Agdi workspace (`~/.agdi/workspace`) if you want to avoid per-folder grants.
