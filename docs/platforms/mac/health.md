...
- App runs `agdi health --json` via `ShellExecutor` every ~60s and on demand. The probe loads creds and reports status without sending messages.
- Cache the last good snapshot and the last error separately to avoid flicker; show the timestamp of each.

## When in doubt

- You can still use the CLI flow in [Gateway health](/gateway/health) (`agdi status`, `agdi status --deep`, `agdi health --json`) and tail `/tmp/agdi/agdi-*.log` for `web-heartbeat` / `web-reconnect`.
