---
summary: "Agdi macOS companion app (menu bar + gateway broker)"
read_when:
...
# Agdi macOS Companion (menu bar + gateway broker)

The macOS app is the **menu‑bar companion** for Agdi. It owns permissions,
...
- Installs the global CLI (`agdi`) via npm/pnpm on request (bun not recommended for the Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `agdi gateway install`.
...
launchctl bootout gui/$UID/ai.agdi.gateway
```

Replace the label with `ai.agdi.<profile>` when running a named profile.
...
The app registers the `agdi://` URL scheme for local actions.
...
```bash
open 'agdi://agent?message=Hello%20from%20deep%20link'
```
...
Avoid putting your Agdi state dir in iCloud or other cloud-synced folders.
...
```bash
cd apps/macos
swift run agdi-mac connect --json
swift run agdi-mac discover --timeout 3000 --json
```
...
Tip: compare against `agdi gateway discover --json` to see whether the
