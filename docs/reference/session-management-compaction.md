...
This document explains how Agdi manages sessions end-to-end:
...
Agdi is designed around a single **Gateway process** that owns session state.
...
Agdi persists sessions in two layers:
...
- Transcripts: `~/.agdi/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Agdi resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Agdi reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.agdi/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Agdi intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Agdi consumes the events, but Pi decides when to compact).
...
Agdi also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Agdi bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Agdi leaves it alone.
...
Agdi supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Agdi strips/suppresses this in the delivery layer.

As of `2026.1.10`, Agdi also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Agdi uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Agdi’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `agdi status`.
