...
Agdi integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Agdi owns lifecycle when `channels.signal.autoStart=true`.
...
- Agdi spawns `imsg rpc` as a child process (legacy iMessage integration).
