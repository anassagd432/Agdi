---
summary: "How Agdi builds prompt context and reports token usage + costs"
read_when:
...
Agdi tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Agdi assembles its own system prompt on every run. It includes:
...
For images, Agdi downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Agdi session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `agdi status --usage` and `agdi channels list` show
...
`cacheWrite`. If pricing is missing, Agdi shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Agdi can
...
Anthropic's 1M context window is currently beta-gated. Agdi can inject the
...
Agdi skips the `context-1m-*` beta header because Anthropic currently
