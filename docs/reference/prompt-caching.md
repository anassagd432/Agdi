...
- With Anthropic API-key auth profiles, Agdi seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Agdi injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Agdi exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.agdi/logs/cache-trace.jsonl" # optional
