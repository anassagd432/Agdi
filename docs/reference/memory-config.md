...
summary: "Full configuration reference for Agdi memory search, embedding providers, QMD backend, hybrid search, and multimodal memory"
read_when:
...
This page covers the full configuration surface for Agdi memory search. For
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, Agdi auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. Agdi
...
BM25 + vectors + reranking. Markdown stays the source of truth; Agdi shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.agdi/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.agdi/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, Agdi retries with `qmd query`. If QMD fails or the binary is
  missing, Agdi automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- Agdi does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - Agdi sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index Agdi
    uses), run a one-off query with the agent's XDG dirs.

    Agdi's QMD state lives under your **state dir** (defaults to `~/.agdi`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    Agdi uses:

    ```bash
    # Pick the same state dir Agdi uses
    STATE_DIR="${OPENCLAW_STATE_DIR:-$HOME/.agdi}"
...
- When `scope` denies a search, Agdi logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, Agdi exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.agdi/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- If `memorySearch.multimodal.enabled = true`, Agdi also indexes supported image/audio files under `extraPaths` only. Default memory roots (`MEMORY.md`, `memory.md`, `memory/**/*.md`) stay Markdown-only.
- Symlinks are ignored (files or directories).

## Multimodal memory files (Gemini image + audio)

Agdi can index image and audio files from `memorySearch.extraPaths` when using Gemini embedding 2:
...
> Agdi will automatically reindex when it detects a model or dimension change.
...
- Index storage: per-agent SQLite at `~/.agdi/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
- Freshness: watcher on `MEMORY.md` + `memory/` marks the index dirty (debounce 1.5s). Sync is scheduled on session start, on search, or on an interval and runs asynchronously. Session transcripts use delta thresholds to trigger background sync.
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, Agdi automatically resets and reindexes the entire store.

## Hybrid search (BM25 + vector)

When enabled, Agdi combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, Agdi falls back to vector-only search.
...
Agdi can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.agdi/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, Agdi stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, Agdi logs the
