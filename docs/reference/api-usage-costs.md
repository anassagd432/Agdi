...
AGDI features that can generate provider usage or paid API calls.
...
- `agdi status --usage` and `agdi channels list` show provider **usage windows**
...
AGDI can pick up credentials from:
...
`agdi models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
