...
<AccordionGroup>
  ...
  <Accordion title="Recommended way to install and set up Agdi">
  ...
  <Accordion title="How do I authenticate the dashboard (token) on localhost vs remote?">
    ...
    **Not on localhost:**
    ...
    - **Tailscale Serve** (recommended): keep bind loopback, run `agdi gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    2. Copy `$OPENCLAW_STATE_DIR` (default: `~/.agdi`) from the old machine.
  ...
  <Accordion title="Cannot access docs.agdi.ai (SSL error)">
    ...
    detail: [Troubleshooting](/help/faq#cannot-access-docsagdi-ai-ssl-error).
  ...
  <Accordion title="Windows install says git not found or agdi not recognized">
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Agdi, track/report it in:

    - [Issue #30640](https://github.com/agdi/agdi/issues/30640)
  ...
  <Accordion title="How do I install Agdi on Linux?">
  ...
  <Accordion title="How do I install Agdi on a VPS?">
  ...
  <Accordion title="Can I ask Agdi to update itself?">
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    ...
    **local-only models** so your data stays on your device. Subscriptions (Claude
    ...
    OpenAI Codex OAuth is explicitly supported for external tools like Agdi.
  ...
  <Accordion title="How does Anthropic setup-token auth work?">
    `claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in onboarding or paste it with `agdi models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
  </Accordion>
  ...
  <Accordion title="How does Codex auth work?">
    Agdi supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Onboarding (CLI)](/start/wizard).
  </Accordion>

  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Agdi fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Agdi. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    ...
    1. Enable the plugin: `agdi plugins enable google`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Agdi needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="If I buy a Mac mini to run Agdi, can I connect it to my MacBook Pro?">
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Onboarding accepts `@username` input and resolves it to a numeric ID, but Agdi authorization uses numeric IDs only.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Agdi instances?">
  ...
  <Accordion title="How important is it to run Agdi on a dedicated machine?">
  ...
  <Accordion title="Can I run Agdi in a VM and what are the requirements?">
...
<AccordionGroup>
  <Accordion title="What is Agdi, in one paragraph?">
  ...
  <Accordion title="What are the top five everyday use cases for Agdi?">
  ...
  <Accordion title="Can Agdi help with lead gen, outreach, ads, and blogs for a SaaS?">
...
<AccordionGroup>
  ...
  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.agdi/agdi.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.agdi/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Agdi treats as `<workspace>/skills` on the next session.
  </Accordion>
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `agdi skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.com](https://clawhub.com).

    ```bash
    agdi skills search "calendar"
    agdi skills install <skill-slug>
  ...
  <Accordion title="Can Agdi run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.agdi/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with Agdi?">
    Use the built-in `user` browser profile, which attaches through Chrome DevTools MCP:

    ```bash
    agdi browser --browser-profile user tabs
    agdi browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  <Accordion title="Is all data used with Agdi saved locally?">
  ...
  <Accordion title="Where does Agdi store its data?">
    Everything lives under `$OPENCLAW_STATE_DIR` (default: `~/.agdi`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$OPENCLAW_STATE_DIR/agdi.json`                             | Main config (JSON5)                                                |
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.agdi` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall Agdi?">
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Agdi reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.agdi/agdi.json`):
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Agdi enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `agdi doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Agdi.` every time.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `agdi configure --section web` and choose a provider.
  ...
  <Accordion title="Can the Agdi browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="Can two Agdi instances talk to each other (local + VPS)?">
...
<AccordionGroup>
  <Accordion title="How does Agdi load environment variables?">
    Agdi reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.agdi/.env` (aka `$OPENCLAW_STATE_DIR/.env`)
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Agdi instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Agdi but keep it installed?">
    ...
    - If you used profiles (`--profile` / `OPENCLAW_PROFILE`), reset each state dir (defaults are `~/.agdi-<profile>`).
  ...
  <Accordion title="Why does Agdi not reply in a group?">
    Two common causes:
...
<AccordionGroup>
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `agdi onboard` and choose `Ollama`
    ...
    - for manual switching, use `agdi models list` and `agdi models set ollama/<model>`
  ...
  <Accordion title="What do Agdi, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `agdi models status`.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current Agdi release (or run from source `main`), then restart the gateway.
...
<AccordionGroup>
  ...
  <Accordion title='Why does agdi gateway status say "Runtime: running" but "RPC probe: failed"?'>
  ...
  <Accordion title='Why does agdi gateway status show "Config (cli)" and "Config (service)" different?'>
  ...
  <Accordion title="How do I run Agdi in remote mode (client connects to a Gateway elsewhere)?">
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `agdi devices list`
      - `agdi devices rotate --device <id> --role operator`
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    Profiles also suffix service names (`ai.agdi.<profile>`; legacy `com.agdi.*`, `agdi-gateway-<profile>.service`, `Agdi Gateway (<profile>)`).
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS: `$OPENCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.agdi/logs/...`; profiles use `~/.agdi-<profile>/logs/...`)
  ...
  <Accordion title="I closed my terminal on Windows - how do I restart Agdi?">
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Agdi already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: agdi gateway restart vs agdi gateway">
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Agdi to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `agdi pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `agdi pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
