---
summary: "Frequently asked questions about AGDI setup, configuration, and usage"
title: "FAQ"
---
...
- [Quick start and first-run setup]
  ...
  - [What's the recommended way to install and set up AGDI?](#whats-the-recommended-way-to-install-and-set-up-agdi)
  ...
  - [I can't access docs.agdi.ai (SSL error). What now?](#i-cant-access-docsagdiai-ssl-error-what-now)
  - [What's the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
  - [How do I install the beta version, and what's the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or agdi not recognized](#windows-install-says-git-not-found-or-agdi-not-recognized)
  - [The docs didn't answer my question - how do I get a better answer?](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [How do I install AGDI on Linux?](#how-do-i-install-agdi-on-linux)
  - [How do I install AGDI on a VPS?](#how-do-i-install-agdi-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask AGDI to update itself?](#can-i-ask-agdi-to-update-itself)
  ...
  - [If I buy a Mac mini to run AGDI, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-agdi-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different AGDI instances?](#can-multiple-people-use-one-whatsapp-number-with-different-agdi-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What's the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run AGDI on a dedicated machine?](#how-important-is-it-to-run-agdi-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run AGDI in a VM and what are the requirements](#can-i-run-agdi-in-a-vm-and-what-are-the-requirements)
- [What is AGDI?](#what-is-agdi)
  - [What is AGDI, in one paragraph?](#what-is-agdi-in-one-paragraph)
  - [What's the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for AGDI](#what-are-the-top-five-everyday-use-cases-for-agdi)
  - [Can AGDI help with lead gen outreach ads and blogs for a SaaS](#can-agdi-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can AGDI run tasks on a schedule or continuously in the background?](#can-agdi-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with AGDI saved locally?](#is-all-data-used-with-agdi-saved-locally)
  - [Where does AGDI store its data?](#where-does-agdi-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What's the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall AGDI?](#how-do-i-completely-uninstall-agdi)
...
- [Config basics](#config-basics)
  ...
  - [Can the AGDI browser run headless?](#can-the-agdi-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two AGDI instances talk to each other (local + VPS)?](#can-two-agdi-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does AGDI load environment variables?](#how-does-agdi-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of AGDI instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-agdi-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset AGDI but keep it installed?](#how-do-i-completely-reset-agdi-but-keep-it-installed)
  ...
  - [Why doesn't AGDI reply in a group?](#why-doesnt-agdi-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do AGDI, Flawd, and Krill use for models?](#what-do-agdi-flawd-and-krill-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `agdi gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-agdi-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `agdi gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-agdi-gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run AGDI in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-agdi-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart AGDI?](#i-closed-my-terminal-on-windows-how-do-i-restart-agdi)
  ...
  - [ELI5: `agdi gateway restart` vs `agdi gateway`](#eli5-agdi-gateway-restart-vs-agdi-gateway)
  - [What's the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media and attachments](#media-and-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose AGDI to inbound DMs?](#is-it-safe-to-expose-agdi-to-inbound-dms)
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/agdi/agdi-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   agdi health --json
   agdi health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://agdi.ai/install.sh | bash -s -- --install-method git
```

This installs AGDI **from a git checkout**, so the agent can read the code + docs and
...
[https://github.com/agdi/agdi/issues](https://github.com/agdi/agdi/issues)
[https://github.com/agdi/agdi/pulls](https://github.com/agdi/agdi/pulls)
...
- `agdi status`: quick snapshot of gateway/agent health + basic config.
- `agdi models status`: checks provider auth + model availability.
- `agdi doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `agdi status --all`, `agdi logs --follow`,
`agdi gateway status`, `agdi health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### What's the recommended way to install and set up AGDI
...
If you don't have a global install yet, run it via `pnpm agdi onboard`.
...
- If it asks for auth, paste the token from `gateway.auth.token` (or `AGDI_GATEWAY_TOKEN`) into Control UI settings.
- Retrieve it from the gateway host: `agdi config get gateway.auth.token` (or generate one: `agdi doctor --generate-gateway-token`).

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `agdi gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy auth (no token).
- **Tailnet bind**: run `agdi gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
1. Install AGDI on the new machine.
2. Copy `$AGDI_STATE_DIR` (default: `~/.agdi`) from the old machine.
3. Copy your workspace (default: `~/.agdi/workspace`).
4. Run `agdi doctor` and restart the Gateway service.
...
under `~/.agdi/` (for example `~/.agdi/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-agdi-store-its-data),
...
[https://github.com/agdi/agdi/blob/main/CHANGELOG.md](https://github.com/agdi/agdi/blob/main/CHANGELOG.md)

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I cant access docs.agdi.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.agdi.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.agdi.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsagdiai-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

If you still can't reach the site, the docs are mirrored on GitHub:
[https://github.com/agdi/agdi/tree/main/docs](https://github.com/agdi/agdi/tree/main/docs)
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://agdi.ai/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://agdi.ai/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
[https://agdi.ai/install.ps1](https://agdi.ai/install.ps1)
...
```bash
curl -fsSL https://agdi.ai/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://agdi.ai/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://agdi.ai/install.sh | bash -s -- --install-method git --verbose
```
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://agdi.ai/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```

More options: [Installer flags](/install/installer).

### Windows install says git not found or agdi not recognized
...
**2) agdi is not recognized after install**
...
### How do I install AGDI on Linux
...
### How do I install AGDI on a VPS
...
### Can I ask AGDI to update itself
...
agdi update
agdi update status
agdi update --channel stable|beta|dev
agdi update --tag <dist-tag|version>
agdi update --no-restart
```

If you must automate from an agent:

```bash
agdi update --yes --no-restart
agdi gateway restart
```
...
`agdi onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run AGDI with **API keys** (Anthropic/OpenAI/others) or with
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `agdi models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `agdi models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `agdi models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. AGDI no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so AGDI can keep replying while a provider is rate-limited.
...
AGDI supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.3-codex` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. AGDI fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `agdi.json`.

Steps:

1. Enable the plugin: `agdi plugins enable google-gemini-cli-auth`
2. Login: `agdi models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. AGDI needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. AGDI runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run AGDI can I connect it to my MacBook Pro
...
- Use `agdi nodes status` / `agdi nodes list` to see it.
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but AGDI authorization uses numeric IDs only.

Safer (no third-party bot):

- DM your bot, then run `agdi logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different AGDI instances
...
If you run AGDI via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the AGDI code install. Your state
(`~/.agdi`) and workspace (`~/.agdi/workspace`) stay untouched.
...
**AGDI-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run AGDI on a dedicated machine
...
AGDI is lightweight. For a basic Gateway + one chat channel:
...
### Can I run AGDI in a VM and what are the requirements
...
### What is AGDI in one paragraph

AGDI is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

AGDI is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for AGDI
...
### Can AGDI help with lead gen outreach ads and blogs for a SaaS
...
AGDI draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

AGDI is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use AGDI when you
...
Showcase: [https://agdi.ai/showcase](https://agdi.ai/showcase)

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.agdi/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.agdi/agdi.json`). Precedence is `<workspace>/skills` > `~/.agdi/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.agdi/agdi.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.agdi/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which AGDI treats as `<workspace>/skills`.
...
- Confirm cron is enabled (`cron.enabled`) and `AGDI_SKIP_CRON` is not set.
...
### Can AGDI run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.agdi.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-agdi-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. AGDI can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/.agdi/skills`) to allow Linux:

   ```markdown
   ---
   name: apple-notes
   description: Manage Apple Notes via the memo CLI on macOS.
   metadata: { "agdi": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured AGDI workspace); AGDI treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.agdi/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
- Persist `/home/node` with `AGDI_HOME_VOLUME` so caches survive.
- Bake system deps into the image with `AGDI_DOCKER_APT_PACKAGES`.
...
AGDI memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

AGDI also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, AGDI auto-selects a provider when it
can resolve an API key (auth profiles, `models.providers.*.apiKey`, or env vars).
It prefers OpenAI if an OpenAI key resolves, otherwise Gemini if a Gemini key
resolves. If neither key is available, memory search stays disabled until you
configure it. If you have a local model path configured and present, AGDI
prefers `local`.
...
### Is all data used with AGDI saved locally

No - **AGDI's state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.agdi` + your workspace directory).
...
### Where does AGDI store its data
...
| `$AGDI_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)      |
| `$AGDI_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                        |
| `$AGDI_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                     |
| `$AGDI_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                 |

Legacy single-agent path: `~/.agdi/agent/*` (migrated by `agdi doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.agdi/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.agdi`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.agdi`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.agdi/skills`).

Default workspace is `~/.agdi/workspace`, configurable via:

```json5
{
  agents: { defaults: { workspace: "~/.agdi/workspace" } },
}
```
...
Do **not** commit anything under `~/.agdi` (credentials, sessions, tokens).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall AGDI
...
`workspace` to the repo root. The AGDI repo is just source code; keep the
...
AGDI reads an optional **JSON5** config from `$AGDI_CONFIG_PATH` (default: `~/.agdi/agdi.json`):

```
$AGDI_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.agdi/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `AGDI_GATEWAY_TOKEN`).
...
If you **really** want open loopback, remove `gateway.auth` from your config. Doctor can generate a token for you any time: `agdi doctor --generate-gateway-token`.
...
key. **Recommended:** run `agdi configure --section web` to store it in
...
- Daemons read env vars from `~/.agdi/.env` (or the service environment).
...
### Can the AGDI browser run headless
...
- Gateway is running: `agdi gateway status`
- Gateway health: `agdi status`
- Channel health: `agdi channels status`
...
### Can two AGDI instances talk to each other local VPS
...
`agdi agent --message ... --deliver`, targeting a chat where the other bot
...
```bash
agdi agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
- Restore from backup (git or a copied `~/.agdi/agdi.json`).
- If you have no backup, re-run `agdi doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use `agdi config set` for small changes.
- Use `agdi configure` for interactive edits.
...
### How does AGDI load environment variables

AGDI reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.agdi/.env` (aka `$AGDI_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.agdi/.env` so they're picked up even when the service doesn't inherit your shell env.
...
`AGDI_LOAD_SHELL_ENV=1`, `AGDI_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`agdi models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means AGDI won't load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
environment. Fix by doing one of these:

1. Put the token in `~/.agdi/.env`:
...
### Is there a way to make a team of AGDI instances one CEO and many agents
...
### How do I completely reset AGDI but keep it installed
...
```bash
agdi reset --scope full --yes --non-interactive
```
...
- If you used profiles (`--profile` / `AGDI_PROFILE`), reset each state dir (defaults are `~/.agdi-<profile>`).
- Dev reset: `agdi gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), AGDI skips the heartbeat run to save API calls.
...
No. AGDI runs on **your own account**, so if you're in the group, AGDI can see it.
...
```bash
agdi directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesnt AGDI reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.agdi/agents/<agentId>/sessions/`.
...
- Use `agdi doctor` to spot stray workspaces and profile mismatches.
...
AGDI's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, AGDI currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `agdi models set ...` (updates just model config)
- `agdi configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.agdi/agdi.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `agdi doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do AGDI, Flawd, and Krill use for models

- **AGDI + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-6`) - see [Anthropic](/providers/anthropic).
...
Yes. AGDI ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
```
~/.agdi/agents/<agentId>/agent/auth-profiles.json
```

Fix options:

- Run `agdi agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so AGDI can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.agdi/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.agdi/agent/*` (migrated by `agdi doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.agdi/.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use `agdi models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `agdi models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `agdi models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.agdi/.env` on the **gateway host**.
  - Clear any pinned order that forces a missing profile:

    ```bash
    agdi models auth order clear --provider anthropic
    ```
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), AGDI will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: AGDI now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
AGDI uses provider-prefixed IDs like:
...
AGDI may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `agdi models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

You can also set a **per-agent** order override (stored in that agent's `auth-profiles.json`) via the CLI:

```bash
# Defaults to the configured default agent (omit --agent)
agdi models auth order get --provider anthropic

# Lock rotation to a single profile (only try this one)
agdi models auth order set --provider anthropic anthropic:default

# Or set an explicit order (fallback within provider)
agdi models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
agdi models auth order set --provider anthropic --agent main anthropic:default
```
...
```
--port > AGDI_GATEWAY_PORT > gateway.port > default 18789
```

### Why does agdi gateway status say Runtime running but RPC probe failed

Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `agdi gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does agdi gateway status show Config cli and Config service different

You're editing one config file while the service is running another (often a `--profile` / `AGDI_STATE_DIR` mismatch).

Fix:

```bash
agdi gateway install --force
```
...
AGDI enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `agdi gateway --port <port>`.

### How do I run AGDI in remote mode client connects to a Gateway elsewhere
...
- `agdi gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- The Control UI stores the token in browser localStorage key `agdi.control.settings.v1`.

Fix:

- Fastest: `agdi dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
- If you don't have a token yet: `agdi doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
- Set `gateway.auth.token` (or `AGDI_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token.
- Still stuck? Run `agdi status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `AGDI_CONFIG_PATH` (per-instance config)
- `AGDI_STATE_DIR` (per-instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use `agdi --profile <name> …` per instance (auto-creates `~/.agdi-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `agdi --profile <name> gateway install`.

Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.agdi.*`, `agdi-gateway-<profile>.service`, `AGDI Gateway (<profile>)`).
...
```
agdi tui --url ws://<host>:18789 --token <token>
```
...
- macOS: `$AGDI_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.agdi/logs/...`; profiles use `~/.agdi-<profile>/logs/...`)
- Linux: `journalctl --user -u agdi-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "AGDI Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, `agdi gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart AGDI
...
1. Is the Gateway running? `agdi gateway status`
2. Is the Gateway healthy? `agdi status`
3. Does the UI have the right token? `agdi dashboard`
...
```bash
agdi channels status
agdi channels logs --channel telegram
```
...
### ELI5 agdi gateway restart vs agdi gateway

- `agdi gateway restart`: restarts the **background service** (launchd/systemd).
- `agdi gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `agdi gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [AGDI assistant setup](/start/agdi) and [Agent send](/tools/agent-send).

CLI sending:

```bash
agdi message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose AGDI to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `agdi pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `agdi pairing list <channel>` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `agdi doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. AGDI only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
agdi pairing approve whatsapp <code>
```

List pending requests:

```bash
agdi pairing list whatsapp
```
...
AGDI blocks **cross-provider** messaging by default. If a tool call is bound
