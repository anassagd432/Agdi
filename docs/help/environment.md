...
3. **Global `.env`** at `~/.agdi/.env` (aka `$OPENCLAW_STATE_DIR/.env`; does not override).
...
On Linux, Agdi automatically detects nvm and applies the fix in the actual startup environment:
...
Export the variable before starting Agdi:
...
Do not rely on writing only to `~/.agdi/.env` for this variable; Node reads
