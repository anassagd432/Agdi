...
This document describes how AGDI integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

AGDI uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, AGDI directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
├── pi-tools.ts                    # createAGDICodingTools()
...
├── agdi-tools.ts              # AGDI-specific tools
...
2. **Custom Replacements**: AGDI replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **AGDI Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures AGDI's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, AGDI CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
AGDI wraps this with `guardSessionManager()` for tool result safety.
...
AGDI maintains an auth profile store with multiple API keys per provider:
...
AGDI loads custom pi extensions for specialized behavior:
...
AGDI also has a local TUI mode that uses pi-tui components directly:
...
- `src/agents/pi-tools.create-agdi-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-agdi-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-agdi-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-agdi-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
