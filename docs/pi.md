...
summary: "Architecture of Agdi's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Agdi
...
Image injection is prompt-local: Agdi loads image refs from the current prompt and
...
| Session storage | `~/.pi/agent/sessions/` | `~/.agdi/agents/<agentId>/sessions/` (or `$OPENCLAW_STATE_DIR/agents/<agentId>/sessions/`) |
