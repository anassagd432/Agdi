...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in AGDI
...
Running AGDI via Node with `tsx` fails at startup with:

```
[agdi] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `agdi status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch agdi.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node agdi.mjs status` works on Node 25.
