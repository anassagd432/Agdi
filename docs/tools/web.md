...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Agdi plugin SDK" });
    ```
...
If no `provider` is set, Agdi checks for API keys in this order and uses
the first one found:
...
<Note>
  ...
  Agdi resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
<Tabs>
  <Tab title="Config file">
    Run `agdi configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.agdi/.env`.
