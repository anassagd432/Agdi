---
summary: "Chrome extension: let AGDI drive your existing Chrome tab"
read_when:
...
The AGDI Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate agdi-managed Chrome profile.
...
AGDI then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the AGDI release (npm package) as static files. There is no separate “build” step.

After upgrading AGDI:

- Re-run `agdi browser extension install` to refresh the installed files under your AGDI state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

AGDI ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want AGDI to control.
...
- `ON`: attached; AGDI can drive that tab.
...
Debugging: `agdi sandbox explain`
...
`agdi browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `agdi browser extension install` first to copy the extension to a stable location under your AGDI state directory.
...
- **This is not isolated** like the dedicated agdi-managed profile.
