...
- For manual spawns, delivery is resilient:
  - Agdi tries direct `agent` delivery first with a stable idempotency key.
...
- Default run timeout: if `sessions_spawn.runTimeoutSeconds` is omitted, Agdi uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
2. Agdi creates or binds a thread to that session target in the active channel.
...
- Otherwise delivery depends on requester depth:
  ...
  - if a nested requester subagent session is gone, Agdi falls back to that session's requester when available
