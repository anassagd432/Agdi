...
- **gateway host** → `agdi` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Agdi also tries to bind
...
When `tools.exec.strictInlineEval=true`, Agdi treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.agdi/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `agdi approvals allowlist ...`).
- `agdi security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `agdi doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Agdi still rejects the `env` builtin in safe-bin
...
`~/.agdi/exec-approvals.json` directly.

CLI: `agdi approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Agdi does not treat
...
topics, Agdi preserves the topic for the approval prompt and the post-approval follow-up.
