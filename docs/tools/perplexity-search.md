...
Agdi supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Agdi also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `agdi configure --section web`. It stores the key in
`~/.agdi/agdi.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.agdi/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
