...
read_when:
  - Running Agdi Gateway in WSL2 while Chrome lives on Windows
...
- Agdi Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/agdi/agdi/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Agdi and Chrome are on the same machine
...
- Agdi points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Agdi is not the problem yet.
...
Fix that before touching Agdi config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Agdi at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Agdi to succeed
...
```bash
agdi browser open https://example.com --browser-profile remote
agdi browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `agdi browser tabs` returns the target
...
3. Agdi config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Agdi config or Control UI auth
