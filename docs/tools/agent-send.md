---
summary: "Direct `agdi agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `agdi agent` (direct agent runs)

`agdi agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `agdi message --target`).
...
agdi agent --to +15555550123 --message "Summon reply" --deliver
