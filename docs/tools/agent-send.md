...
`agdi agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    agdi agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    agdi agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    agdi agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    agdi agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    agdi agent --agent ops --message "Generate report" \
...
agdi agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
