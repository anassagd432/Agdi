...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Agdi run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask Agdi in plain language to "run this in Codex" or "start Claude Code in a thread", Agdi should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Agdi-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Agdi native sub-agent runtime  |
...
- Agdi binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Agdi returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning Agdi agent id.
...
- Agdi ensures the configured ACP session exists before use.
...
- If `agentId` is omitted, Agdi uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Agdi may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Agdi session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the Agdi sandbox.
...
If no target resolves, Agdi returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Agdi returns a clear unsupported-control error.
...
When Agdi uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Agdi `agentId` path).
...
```bash
agdi plugins install acpx
agdi config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
agdi plugins install ./extensions/acpx
```
...
- Relative paths resolve from Agdi workspace directory.
...
- Agdi startup remains non-blocking while the backend health check runs.
...
```bash
agdi config set plugins.entries.acpx.config.permissionMode approve-all
agdi config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** Agdi currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
