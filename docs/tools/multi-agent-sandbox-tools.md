...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `agdi sandbox explain`.
...
Legacy `agent.*` configs are migrated by `agdi doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=agdi-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${AGDI_STATE_DIR:-$HOME/.agdi}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
