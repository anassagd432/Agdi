...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `agdi sandbox explain`.
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENCLAW_STATE_DIR:-$HOME/.agdi}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
