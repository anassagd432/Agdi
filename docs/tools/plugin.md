---
summary: "Install, configure, and manage Agdi plugins"
read_when:
...
Plugins extend Agdi with new capabilities: channels, model providers, tools,
skills, speech, image generation, and more. Some plugins are **core** (shipped
with Agdi), others are **external** (published on npm by the community).
...
<Steps>
  ...
  <Step title="Install a plugin">
    ...
    agdi plugins install ./my-plugin.tgz
    ```
...
Agdi recognizes two plugin formats:
...
watch + in-process restart enabled (the default `agdi gateway` path), that
...
Agdi scans for plugins in this order (first match wins):
...
<Steps>
  ...
  <Step title="Workspace extensions">
    `\<workspace\>/.agdi/extensions/*.ts` and `\<workspace\>/.agdi/extensions/*/index.ts`.
  </Step>

  <Step title="Global extensions">
    `~/.agdi/extensions/*.ts` and `~/.agdi/extensions/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with Agdi. Many are enabled by default (model providers, speech).
...
```bash
agdi plugins list                    # compact inventory
agdi plugins inspect <id>            # deep detail
agdi plugins inspect <id> --json     # machine-readable
agdi plugins status                  # operational summary
agdi plugins doctor                  # diagnostics

agdi plugins install <package>        # install (ClawHub first, then npm)
agdi plugins install clawhub:<pkg>   # install from ClawHub only
agdi plugins install <path>          # install from local path
agdi plugins install -l <path>       # link (no copy) for dev
agdi plugins update <id>             # update one plugin
agdi plugins update --all            # update all

agdi plugins enable <id>
agdi plugins disable <id>
```

See [`agdi plugins` CLI reference](/cli/plugins) for full details.
