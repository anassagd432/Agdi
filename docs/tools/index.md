---
summary: "Agent tool surface for AGDI (browser, canvas, nodes, message, cron) replacing legacy `agdi-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `agdi-*` skills
title: "Tools"
---

# Tools (AGDI)

AGDI exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `agdi-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `agdi.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, AGDI logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:agdi`: all built-in AGDI tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`agdi node run`).
...
- Requires a Brave API key (recommended: `agdi configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated AGDI-managed browser.
...
- Quick smoke: `agdi nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `agdi gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, AGDI clamps `tools.sessions.visibility` to `tree`.
