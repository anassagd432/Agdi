...
Agdi can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `agdi configure --section web` enables the bundled Firecrawl plugin automatically.
...
Agdi always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
