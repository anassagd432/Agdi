...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/agdi`)
...
If the page fails to load, start the Gateway first: `agdi gateway`.
...
created. Re-run `agdi devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `agdi devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.agdi/agdi.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop agdi`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Agdi
...
Agdi **blocks** Control UI connections without device identity.
