...
The fastest way to install. It detects your OS, installs Node if needed, installs Agdi, and launches onboarding.
...
Or skip the link and use `pnpm agdi ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
agdi --version      # confirm the CLI is available
agdi doctor         # check for config issues
agdi gateway status # verify the Gateway is running
```

## Hosting and deployment

Deploy Agdi on a cloud server or VPS:
...
If the install succeeded but `agdi` is not found in your terminal:
