---
summary: "Host Agdi on a Raspberry Pi for always-on self-hosting"
read_when:
...
Run a persistent, always-on Agdi Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well.
...
**Service will not start** -- Check logs with `journalctl -u agdi --no-pager -n 100` and run `agdi doctor --non-interactive`.
