---
summary: "Deploy Agdi on Render with Infrastructure-as-Code"
read_when:
  - Deploying Agdi to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Agdi on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/agdi/agdi)
...
repository configures everything needed to run Agdi:
...
If you use the original Agdi repository, Render will not auto-deploy your Agdi. To update it, run a manual Blueprint sync from the dashboard.
...
For Agdi, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive you can restore on any Agdi host. See [Backup](/cli/backup) for details.
...
regularly export your config via `agdi backup create` in the Render shell.
