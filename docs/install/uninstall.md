...
- If you used profiles (`--profile` / `OPENCLAW_PROFILE`), repeat step 3 for each state dir (defaults are `~/.agdi-<profile>`).
...
Default label is `ai.agdi.gateway` (or `ai.agdi.<profile>`; legacy `com.agdi.*` may still exist):

```bash
launchctl bootout gui/$UID/ai.agdi.gateway
rm -f ~/Library/LaunchAgents/ai.agdi.gateway.plist
```

If you used a profile, replace the label and plist name with `ai.agdi.<profile>`. Remove any legacy `com.agdi.*` plists if present.
