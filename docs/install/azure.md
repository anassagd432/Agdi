---
summary: "Run Agdi Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Agdi running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Agdi Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# Agdi on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs Agdi.
...
- Install Agdi with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    VM_SUBNET_NAME="snet-agdi-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-agdi-vm"
    VM_NAME="vm-agdi"
    ADMIN_USERNAME="agdi"
    BASTION_NAME="bas-agdi"
    BASTION_PIP_NAME="pip-agdi-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Agdi (in the VM shell)">
    ```bash
    curl -fsSL https://agdi.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node LTS and dependencies if not already present, installs Agdi, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Agdi onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The Agdi Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on Agdi Azure deployment with the GitHub Copilot model provider: [Agdi on Azure with GitHub Copilot](https://github.com/johnsonshi/agdi-azure-github-copilot)
