---
summary: "Deploy Agdi on Railway with one-click template"
read_when:
  - Deploying Agdi to Railway
  - You want a one-click cloud deploy with browser-based Control UI
title: "Railway"
---

Deploy Agdi on Railway with a one-click template and access it through the web Control UI.
...
5. Open `https://<your-railway-domain>/agdi` and connect using your `OPENCLAW_GATEWAY_TOKEN`.
...
- `https://<your-railway-domain>/agdi` — Control UI
...
- `OPENCLAW_STATE_DIR=/data/.agdi` (recommended)
- `OPENCLAW_WORKSPACE_DIR=/data/workspace` (recommended)

## Connect a channel

Use the Control UI at `/agdi` or run `agdi onboard` via Railway's shell for channel setup instructions:
