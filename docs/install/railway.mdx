...
Deploy AGDI on Railway with a one-click template and finish setup in your browser.
...
- `AGDI_STATE_DIR=/data/.agdi` (recommended)
- `AGDI_WORKSPACE_DIR=/data/workspace` (recommended)
- `AGDI_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your AGDI state + workspace so you can migrate to another host without losing config or memory.
