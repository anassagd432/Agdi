---
summary: "Move (migrate) an Agdi install from one machine to another"
read_when:
...
This guide moves an Agdi gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.agdi/` by default) and your **workspace**, you preserve:

- **Config** -- `agdi.json` and all gateway settings
...
<Tip>
Run `agdi status` on the old machine to confirm your state directory path.
Custom profiles use `~/.agdi-<profile>/` or a path set via `OPENCLAW_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    If you use multiple profiles (e.g. `~/.agdi-work`), archive each separately.

  </Step>

  <Step title="Install Agdi on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.agdi/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    ...
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `agdi doctor`.
  </Accordion>

  <Accordion title="Copying only agdi.json">
  ...
  <Accordion title="Remote mode">
    ...
    Migrate the gateway host itself, not your local laptop. See [FAQ](/help/faq#where-does-agdi-store-its-data).
  </Accordion>
