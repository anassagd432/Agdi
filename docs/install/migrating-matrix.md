---
summary: "How Agdi upgrades the previous Matrix plugin in place, including encrypted-state recovery limits and manual recovery steps."
read_when:
...
- runtime state stays under `~/.agdi/matrix/`

You do not need to rename config keys or reinstall the plugin under a new name.

## What the migration does automatically

When the gateway starts, and when you run [`agdi doctor --fix`](/gateway/doctor), Agdi tries to repair old Matrix state automatically.
Before any actionable Matrix migration step mutates on-disk state, Agdi creates or reuses a focused recovery snapshot.

When you use `agdi update`, the exact trigger depends on how Agdi is installed:

- source installs run `agdi doctor --fix` during the update flow, then restart the gateway by default
...
- if you use `agdi update --no-restart`, startup-backed Matrix migration is deferred until you later run `agdi doctor --fix` and restart the gateway

Automatic migration covers:

- creating or reusing a pre-migration snapshot under `~/Backups/agdi-migrations/`
...
- Agdi writes a marker file at `~/.agdi/matrix/migration-snapshot.json` after a successful snapshot so later startup and repair passes can reuse the same archive.
- These automatic Matrix migration snapshots back up config + state only (`includeWorkspace: false`).
- If Matrix only has warning-only migration state, for example because `userId` or `accessToken` is still missing, Agdi does not create the snapshot yet because no Matrix mutation is actionable.
- If the snapshot step fails, Agdi skips Matrix migration for that run instead of mutating state without a recovery point.

About multi-account upgrades:

- the oldest flat Matrix store (`~/.agdi/matrix/bot-storage.json` and `~/.agdi/matrix/crypto/`) came from a single-store layout, so Agdi can only migrate it into one resolved Matrix account target
...
Agdi cannot automatically recover:
...
- custom Matrix plugin path installs are surfaced by both gateway startup and `agdi doctor`
...
1. Update Agdi and the Matrix plugin normally.
   Prefer plain `agdi update` without `--no-restart` so startup can finish the Matrix migration immediately.
2. Run:
...
4. Check current verification and backup state:
   ...
   ```bash
   agdi matrix verify status
   agdi matrix verify backup status
   ```

5. If Agdi tells you a recovery key is needed, run:

   ```bash
   agdi matrix verify backup restore --recovery-key "<your-recovery-key>"
   ```

6. If this device is still unverified, run:

   ```bash
   agdi matrix verify device "<your-recovery-key>"
   ```

7. If you are intentionally abandoning unrecoverable old history and want a fresh backup baseline for future messages, run:

   ```bash
   agdi matrix verify backup reset --yes
   ```

8. If no server-side key backup exists yet, create one for future recoveries:

   ```bash
   agdi matrix verify bootstrap
   ```
...
1. Startup or `agdi doctor --fix` creates or reuses the pre-migration snapshot if encrypted migration is actionable.
2. Startup or `agdi doctor --fix` inspects the old Matrix crypto store through the active Matrix plugin install.
3. If a backup decryption key is found, Agdi writes it into the new recovery-key flow and marks room-key restore as pending.
4. On the next Matrix startup, Agdi restores backed-up room keys into the new crypto store automatically.

If the old store reports room keys that were never backed up, Agdi warns instead of pretending recovery succeeded.
...
- Meaning: Agdi created a recovery archive before mutating Matrix state.
- What to do: keep the printed archive path until you confirm migration succeeded.

`Matrix migration snapshot reused before applying Matrix upgrades.`

- Meaning: Agdi found an existing Matrix migration snapshot marker and reused that archive instead of creating a duplicate backup.
- What to do: keep the printed archive path until you confirm migration succeeded.

`Legacy Matrix state detected at ... but channels.matrix is not configured yet.`

- Meaning: old Matrix state exists, but Agdi cannot map it to a current Matrix account because Matrix is not configured.
- What to do: configure `channels.matrix`, then rerun `agdi doctor --fix` or restart the gateway.

`Legacy Matrix state detected at ... but the new account-scoped target could not be resolved yet (need homeserver, userId, and access token for channels.matrix...).`

- Meaning: Agdi found old state, but it still cannot determine the exact current account/device root.
- What to do: start the gateway once with a working Matrix login, or rerun `agdi doctor --fix` after cached credentials exist.

`Legacy Matrix state detected at ... but multiple Matrix accounts are configured and channels.matrix.defaultAccount is not set.`

- Meaning: Agdi found one shared flat Matrix store, but it refuses to guess which named Matrix account should receive it.
- What to do: set `channels.matrix.defaultAccount` to the intended account, then rerun `agdi doctor --fix` or restart the gateway.

`Matrix legacy sync store not migrated because the target already exists (...)`

- Meaning: the new account-scoped location already has a sync or crypto store, so Agdi did not overwrite it automatically.
...
- Meaning: Agdi tried to move old Matrix state but the filesystem operation failed.
- What to do: inspect filesystem permissions and disk state, then rerun `agdi doctor --fix`.

`Legacy Matrix encrypted state detected at ... but channels.matrix is not configured yet.`

- Meaning: Agdi found an old encrypted Matrix store, but there is no current Matrix config to attach it to.
- What to do: configure `channels.matrix`, then rerun `agdi doctor --fix` or restart the gateway.

`Legacy Matrix encrypted state detected at ... but the account-scoped target could not be resolved yet (need homeserver, userId, and access token for channels.matrix...).`

- Meaning: the encrypted store exists, but Agdi cannot safely decide which current account/device it belongs to.
- What to do: start the gateway once with a working Matrix login, or rerun `agdi doctor --fix` after cached credentials are available.

`Legacy Matrix encrypted state detected at ... but multiple Matrix accounts are configured and channels.matrix.defaultAccount is not set.`

- Meaning: Agdi found one shared flat legacy crypto store, but it refuses to guess which named Matrix account should receive it.
- What to do: set `channels.matrix.defaultAccount` to the intended account, then rerun `agdi doctor --fix` or restart the gateway.
...
`- Skipping Matrix migration changes for now. Resolve the snapshot failure, then rerun "agdi doctor --fix".`

- Meaning: Agdi refused to mutate Matrix state because it could not create the recovery snapshot first.
- What to do: resolve the backup error, then rerun `agdi doctor --fix` or restart the gateway.

`Failed migrating legacy Matrix client storage: ...`

- Meaning: the Matrix client-side fallback found old flat storage, but the move failed. Agdi now aborts that fallback instead of silently starting with a fresh store.
...
`Legacy Matrix encrypted state for account "..." has backed-up room keys, but no local backup decryption key was found. Ask the operator to run "agdi matrix verify backup restore --recovery-key <key>" after upgrade if they have the recovery key.`

- Meaning: backup exists, but Agdi could not recover the recovery key automatically.
- What to do: run `agdi matrix verify backup restore --recovery-key "<your-recovery-key>"`.

`Failed inspecting legacy Matrix encrypted state for account "..." (...): ...`

- Meaning: Agdi found the old encrypted store, but it could not inspect it safely enough to prepare recovery.
- What to do: rerun `agdi doctor --fix`. If it repeats, keep the old state directory intact and recover using another verified Matrix client plus `agdi matrix verify backup restore --recovery-key "<your-recovery-key>"`.

`Legacy Matrix backup key was found for account "...", but .../recovery-key.json already contains a different recovery key. Leaving the existing file unchanged.`

- Meaning: Agdi detected a backup key conflict and refused to overwrite the current recovery-key file automatically.
...
- What to do: run `agdi matrix verify backup status`, then retry with `agdi matrix verify backup restore --recovery-key "<your-recovery-key>"` if needed.

### Manual recovery messages

`Backup key is not loaded on this device. Run 'agdi matrix verify backup restore' to load it and restore old room keys.`

- Meaning: Agdi knows you should have a backup key, but it is not active on this device.
- What to do: run `agdi matrix verify backup restore`, or pass `--recovery-key` if needed.

`Store a recovery key with 'agdi matrix verify device <key>', then run 'agdi matrix verify backup restore'.`
...
`Backup key mismatch on this device. Re-run 'agdi matrix verify device <key>' with the matching recovery key.`

- Meaning: the stored key does not match the active Matrix backup.
- What to do: rerun `agdi matrix verify device "<your-recovery-key>"` with the correct key.

If you accept losing unrecoverable old encrypted history, you can instead reset the current backup baseline with `agdi matrix verify backup reset --yes`.

`Backup trust chain is not verified on this device. Re-run 'agdi matrix verify device <key>'.`

- Meaning: the backup exists, but this device does not trust the cross-signing chain strongly enough yet.
- What to do: rerun `agdi matrix verify device "<your-recovery-key>"`.
...
- What to do: verify the device first, then recheck with `agdi matrix verify backup status`.

`Matrix crypto backend cannot load backup keys from secret storage. Verify this device with 'agdi matrix verify device <key>' first.`

- Meaning: this device cannot restore from secret storage until device verification is complete.
- What to do: run `agdi matrix verify device "<your-recovery-key>"` first.
...
```bash
agdi matrix verify status --verbose
agdi matrix verify backup status --verbose
agdi matrix verify backup restore --recovery-key "<your-recovery-key>" --verbose
```
