---
summary: "Deploy Agdi Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Agdi on a Kubernetes cluster
  - You want to test Agdi in a Kubernetes environment
title: "Kubernetes"
---

# Agdi on Kubernetes

A minimal starting point for running Agdi on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Agdi is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/agdi 18789:18789 -n agdi
...
```bash
kubectl get secret agdi-secrets -n agdi -o jsonpath='{.data.OPENCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: agdi (configurable via OPENCLAW_NAMESPACE)
├── Deployment/agdi        # Single pod, init container + gateway
├── Service/agdi           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/agdi-config  # agdi.json + AGENTS.md
└── Secret/agdi-secrets    # Gateway token + API keys
```
...
Edit `agdi.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret agdi-secrets -n agdi \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/agdi -n agdi
```
...
```yaml
image: ghcr.io/agdi/agdi:latest # or pin to a specific version from https://github.com/agdi/agdi/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # agdi.json + AGENTS.md
