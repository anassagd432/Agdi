---
summary: "Delegate architecture: running Agdi as a named agent on behalf of an organization"
...
Goal: run Agdi as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Agdi agent that:
...
Agdi's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Agdi's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.agdi/workspace-delegate",
  tools: {
...
- Cron run history: `~/.agdi/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.agdi/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Agdi's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
agdi agents add delegate
```

This creates:

- Workspace: `~/.agdi/workspace-delegate`
- State: `~/.agdi/agents/delegate/agent`
- Sessions: `~/.agdi/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.agdi/workspace" },
      {
        id: "delegate",
        workspace: "~/.agdi/workspace-delegate",
        tools: {
...
~/.agdi/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.agdi/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.agdi/workspace-org",
        agentDir: "~/.agdi/agents/org-assistant/agent",
