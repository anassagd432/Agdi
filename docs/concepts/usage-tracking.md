...
- `/usage cost` in chats: local cost summary aggregated from Agdi session logs.
- CLI: `agdi status --usage` prints a full per-provider breakdown.
- CLI: `agdi channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
