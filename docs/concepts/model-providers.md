...
- CLI helpers: `agdi onboard`, `agdi models list`, `agdi models set <provider/model>`.
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  Agdi merges that output into `models.providers` before writing
  `models.json`.
...
Provider plugins can now own most provider-specific logic while Agdi keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `agdi onboard`, `agdi models auth`, and headless setup
...
That covers providers that still fit Agdi's normal transports. A provider
...
Agdi ships with the pi‑ai catalog. These providers require **no**
...
- CLI: `agdi onboard --auth-choice openai-api-key`
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in Agdi because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- CLI: `agdi onboard --auth-choice token` (paste setup-token) or `agdi models auth paste-token --provider anthropic`
- Direct API-key models support the shared `/fast` toggle and `params.fastMode`; Agdi maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
- CLI: `agdi onboard --auth-choice openai-codex` or `agdi models auth login --provider openai-codex`
- Default transport is `auto` (WebSocket-first, SSE fallback)
...
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like Agdi.
...
- CLI: `agdi onboard --auth-choice opencode-zen` or `agdi onboard --auth-choice opencode-go`
...
- Compatibility: legacy Agdi config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- CLI: `agdi onboard --auth-choice gemini-api-key`

### Google Vertex and Gemini CLI

- Providers: `google-vertex`, `google-gemini-cli`
- Auth: Vertex uses gcloud ADC; Gemini CLI uses its OAuth flow
- Caution: Gemini CLI OAuth in Agdi is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Enable: `agdi plugins enable google`
  - Login: `agdi models auth login --provider google-gemini-cli --set-default`
  - Note: you do **not** paste a client id or secret into `agdi.json`. The CLI login flow stores
...
- CLI: `agdi onboard --auth-choice zai-api-key`
...
- CLI: `agdi onboard --auth-choice ai-gateway-api-key`
...
- CLI: `agdi onboard --kilocode-api-key <key>`
...
- CLI: `agdi onboard --auth-choice mistral-api-key`
- Groq: `groq` (`GROQ_API_KEY`)
- Cerebras: `cerebras` (`CEREBRAS_API_KEY`)
  - GLM models on Cerebras use ids `zai-glm-4.7` and `zai-glm-4.6`.
  - OpenAI-compatible base URL: `https://api.cerebras.ai/v1`.
- GitHub Copilot: `github-copilot` (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`)
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `agdi onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
```bash
agdi models auth login --provider qwen-portal --set-default
```
...
- CLI: `agdi onboard --auth-choice volcengine-api-key`
...
- CLI: `agdi onboard --auth-choice byteplus-api-key`
...
- CLI: `agdi onboard --auth-choice synthetic-api-key`
...
`agdi onboard` and the model picker. See [/providers/ollama](/providers/ollama)
...
- For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), Agdi forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
- If `baseUrl` is empty/omitted, Agdi keeps the default OpenAI behavior (which resolves to `api.openai.com`).
- For safety, an explicit `compat.supportsDeveloperRole: true` is still overridden on non-native `openai-completions` endpoints.

## CLI examples

```bash
agdi onboard --auth-choice opencode-zen
agdi models set opencode/claude-opus-4-6
agdi models list
```
