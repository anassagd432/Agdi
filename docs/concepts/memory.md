...
- `MEMORY.md` (optional)
  ...
  - If both `MEMORY.md` and `memory.md` exist at the workspace root, Agdi loads both (deduplicated by realpath so symlinks pointing to the same file are not injected twice).
...
Agdi exposes two agent-facing tools for these Markdown files:
