...
Agdi selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Agdi can use (plus aliases).
...
agdi models list
agdi models status
agdi models set <provider/model>
agdi models set-image <provider/model>

agdi models aliases list
agdi models aliases add <alias> <provider/model>
agdi models aliases remove <alias>

agdi models fallbacks list
agdi models fallbacks add <provider/model>
agdi models fallbacks remove <provider/model>
agdi models fallbacks clear

agdi models image-fallbacks list
agdi models image-fallbacks add <provider/model>
agdi models image-fallbacks remove <provider/model>
agdi models image-fallbacks clear
```

`agdi models` (no subcommand) is a shortcut for `models status`.
...
`agdi models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.agdi/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Agdi writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Agdi regenerates `models.json`, including command-driven paths like `agdi agent`.
