...
read_when:
  - You want to understand how Agdi assembles model context
...
A **context engine** controls how Agdi builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Agdi ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.agdi/agdi.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Agdi plugin. Install
...
agdi plugins install -l ./my-context-engine
```
...
Every time Agdi runs a model prompt, the context engine participates at
...
Agdi currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Agdi
...
The built-in `legacy` engine preserves Agdi's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Agdi uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Agdi disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Agdi automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `agdi/plugin-sdk/core` to use
  Agdi's built-in compaction behavior.
...
Agdi resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Agdi's built-in summarization. Plugin engines can implement
...
- Use `agdi doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Agdi
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `agdi plugins install -l ./my-engine` to link a
