...
- When given a `sessionId`, Agdi resolves it to the corresponding session key (missing ids error).
...
- After the primary run completes, Agdi runs a **reply-back loop**:
  - Round 2+ alternates between requester and target agents.
  - Reply exactly `REPLY_SKIP` to stop the ping‑pong.
  - Max turns is `session.agentToAgent.maxPingPongTurns` (0–5, default 5).
- Once the loop ends, Agdi runs the **agent‑to‑agent announce step** (target agent only):
...
- Default runtime: Agdi sub-agent (`runtime: "subagent"`).
...
- `agentId?` (optional)
  - `runtime: "subagent"`: target another Agdi agent id if allowed by `subagents.allowAgents`
...
- `attachments?` (optional array of inline files; subagent runtime only, ACP rejects). Each entry: `{ name, content, encoding?: "utf8" | "base64", mimeType? }`. Files are materialized into the child workspace at `.agdi/attachments/<uuid>/`. Returns a receipt with sha256 per file.
- `attachAs?` (optional; `{ mountPath? }` hint reserved for future mount implementations)

Allowlist:

- `runtime: "subagent"`: `agents.list[].subagents.allowAgents` controls which Agdi agent ids are allowed via `agentId` (`["*"]` to allow any). Default: only the requester agent.
...
- After completion, Agdi runs a sub-agent **announce step** and posts the result to the requester chat channel.
...
- When a session is sandboxed and `sessionToolsVisibility="spawned"`, Agdi clamps visibility to `tree` even if you set `tools.sessions.visibility="all"`.
