---
summary: "Use Synthetic's Anthropic-compatible API in AGDI"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. AGDI registers it as the
...
Note: AGDI's Anthropic client appends `/v1` to the base URL, so use
