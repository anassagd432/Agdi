---
summary: "Use MiniMax M2.1 in AGDI"
read_when:
  - You want MiniMax models in AGDI
...
```bash
agdi plugins enable minimax-portal-auth  # skip if already loaded.
agdi gateway restart  # restart if gateway is already running
agdi onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/agdi/agdi/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `agdi.json`:
...
- Use `agdi models list` and `agdi models set minimax/MiniMax-M2.1` to switch.
...
- Running `agdi configure` and selecting **MiniMax M2.1**, or
