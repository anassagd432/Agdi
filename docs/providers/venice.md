---
summary: "Use Venice AI privacy-focused models in Agdi"
read_when:
  - You want privacy-focused inference in Agdi
...
```bash
agdi onboard --auth-choice venice-api-key
```
...
```bash
agdi agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Agdi shows all available Venice models. Pick based on your needs:
...
```bash
agdi models set venice/kimi-k2-5
agdi models set venice/claude-opus-4-6
```

List all available models:

```bash
agdi models list | grep venice
```

## Configure via `agdi configure`

1. Run `agdi configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Agdi automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
agdi agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
agdi agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
agdi agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
agdi agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
agdi agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `agdi models list` to see currently available models. Some models may be temporarily offline.
