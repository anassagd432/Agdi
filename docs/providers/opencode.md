---
summary: "Use OpenCode Zen and Go catalogs with Agdi"
read_when:
...
OpenCode exposes two hosted catalogs in Agdi:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Agdi keeps the runtime provider ids
...
agdi onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
agdi onboard --auth-choice opencode-go
agdi onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
