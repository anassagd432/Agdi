---
summary: "Use OpenAI via API keys or Codex subscription in Agdi"
read_when:
  - You want to use OpenAI models in Agdi
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Agdi.
...
```bash
agdi onboard --auth-choice openai-api-key
# or non-interactive
agdi onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Agdi forwards both through the `openai/*` Responses path.
Agdi intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

Agdi does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in Agdi.
...
agdi onboard --auth-choice openai-codex

# Or run OAuth directly
agdi models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Agdi
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, Agdi also supports:

- `openai-codex/gpt-5.3-codex-spark`

Agdi treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

Agdi also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
Agdi uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Agdi also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Agdi enables it by default for
...
Agdi, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
Agdi exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, Agdi applies a low-latency OpenAI profile:
...
`baseUrl` on `api.openai.com`), Agdi now auto-enables OpenAI server-side
