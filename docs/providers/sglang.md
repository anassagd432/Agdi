---
summary: "Run Agdi with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Agdi against a local SGLang server
...
Agdi can connect to SGLang using the `openai-completions` API.

Agdi can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Agdi will query:
