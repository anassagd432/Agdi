...
read_when:
  - You need detailed behavior for agdi onboard
...
This page is the full reference for `agdi onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.agdi/agdi.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `agdi doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `agdi pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `agdi health`.
    - `agdi status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.agdi/.env` so launchd can read it.
...
- OAuth credentials: `~/.agdi/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.agdi/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.agdi/credentials/oauth.json` (or `$AGDI_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`agdi onboard`](/cli/onboard)
