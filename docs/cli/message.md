---
summary: "CLI reference for `agdi message` (send + channel actions)"
read_when:
...
- On cache miss, Agdi will attempt a live directory lookup when the provider supports it.
...
- `agdi message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Agdi does not force a `default` account SecretRef scope
...
```
agdi message send --channel discord \
...
```
agdi message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
agdi message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
agdi message poll --channel msteams \
...
```
agdi message react --channel slack \
...
```
agdi message react --channel signal \
...
```
agdi message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
agdi message send --channel telegram --target @mychat \
