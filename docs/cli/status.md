---
summary: "CLI reference for `agdi status` (diagnostics, probes, usage snapshots)"
read_when:
...
- Update info surfaces in the Overview; if an update is available, status prints a hint to run `agdi update` (see [Updating](/install/updating)).
