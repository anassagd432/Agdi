---
summary: "CLI reference for `agdi onboard` (interactive onboarding)"
read_when:
...
```bash
agdi onboard
agdi onboard --flow quickstart
agdi onboard --flow manual
agdi onboard --mode remote --remote-url wss://gateway-host:18789
```
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `agdi gateway run`.
...
- Fastest first chat: `agdi dashboard` (Control UI, no channel setup).
