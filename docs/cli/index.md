---
summary: "Agdi CLI reference for `agdi` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.agdi-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.agdi-<name>`.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `agdi update` (source installs only).
...
Note: plugins can add additional top-level commands (for example `agdi voicecall`).

## Security

- `agdi security audit` — audit config + local state for common security foot-guns.
- `agdi security audit --deep` — best-effort live Gateway probe.
- `agdi security audit --fix` — tighten safe defaults and chmod state/config.

## Secrets

- `agdi secrets reload` — re-resolve refs and atomically swap the runtime snapshot.
- `agdi secrets audit` — scan for plaintext residues, unresolved refs, and precedence drift (`--allow-exec` to execute exec providers during audit).
- `agdi secrets configure` — interactive helper for provider setup + SecretRef mapping + preflight/apply (`--allow-exec` to execute exec providers during preflight and exec-containing apply flows).
- `agdi secrets apply --from <plan.json>` — apply a previously generated plan (`--dry-run` supported; use `--allow-exec` to permit exec providers in dry-run and exec-containing write plans).

## Plugins

Manage extensions and their config:

- `agdi plugins list` — discover plugins (use `--json` for machine output).
- `agdi plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `agdi plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `agdi plugins marketplace list <marketplace>` — list marketplace entries before install.
- `agdi plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `agdi plugins doctor` — report plugin load errors.
...
- `agdi memory status` — show index stats.
- `agdi memory index` — reindex memory files.
- `agdi memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
...
- `--workspace <dir>`: agent workspace path (default `~/.agdi/workspace`).
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `agdi config` with no
...
- `config set --dry-run`: validate assignments without writing `agdi.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `agdi.json`.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `agdi health` or `agdi status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `agdi doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, Agdi moves account-scoped values into `channels.<channel>.accounts.default` before writing the new account.
...
```bash
agdi channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
agdi channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
agdi channels remove --channel discord --account work --delete
...
Tip: use `agdi skills search`, `agdi skills install`, and `agdi skills update` for ClawHub-backed skills.
...
- `agdi message send --target +15555550123 --message "Hi"`
- `agdi message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, Agdi may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Agdi can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `agdi status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Agdi services are treated as first-class and aren't flagged as "extra".
...
`agdi models` is an alias for `models status`.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`agdi browser`](/cli/browser) and the [Browser tool](/tools/browser).
