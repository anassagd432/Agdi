---
summary: "CLI reference for `agdi plugins` (list, install, uninstall, enable/disable, doctor)"
read_when:
...
All plugins must ship a `agdi.plugin.json` file with an inline JSON Schema
...
state dir extensions root (`$AGDI_STATE_DIR/extensions/<id>`). Use
...
agdi plugins update <id>
agdi plugins update --all
agdi plugins update <id> --dry-run
```
