---
summary: "CLI reference for `agdi secrets` (reload, audit, configure, apply)"
read_when:
...
Use `agdi secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `agdi.json` refs)
...
agdi secrets audit
agdi secrets audit --check
agdi secrets audit --json
agdi secrets audit --allow-exec
```
...
```bash
agdi secrets configure
agdi secrets configure --plan-out /tmp/agdi-secrets-plan.json
agdi secrets configure --apply --yes
agdi secrets configure --providers-only
agdi secrets configure --skip-provider-setup
agdi secrets configure --agent ops
agdi secrets configure --json
```
...
- `configure` targets secret-bearing fields in `agdi.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Agdi fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
agdi secrets apply --from /tmp/agdi-secrets-plan.json --json
```
...
- `agdi.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.agdi/.env` known secret keys whose values were migrated
