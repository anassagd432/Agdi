---
summary: "CLI reference for `agdi hooks` (agent hooks)"
read_when:
...
Details:
  Source: agdi-bundled
  Path: /path/to/agdi/hooks/bundled/session-memory/HOOK.md
  Handler: /path/to/agdi/hooks/bundled/session-memory/handler.ts
...
```bash
agdi hooks enable <name>
```

Enable a specific hook by adding it to your config (`~/.agdi/config.json`).

**Note:** Workspace hooks are disabled by default until enabled here or in config. Hooks managed by plugins show `plugin:<id>` in `agdi hooks list` and can’t be enabled/disabled here. Enable/disable the plugin instead.
...
`agdi hooks install` still works as a compatibility alias, but it prints a
deprecation warning and forwards to `agdi plugins install`.
...
- Copies the hook pack into `~/.agdi/hooks/<id>`
...
```bash
# Local directory
agdi plugins install ./my-hook-pack

# Local archive
agdi plugins install ./my-hook-pack.zip

# NPM package
agdi plugins install @agdi/my-hook-pack

# Link a local directory without copying
agdi plugins install -l ./my-hook-pack
```
...
`agdi hooks update` still works as a compatibility alias, but it prints a
deprecation warning and forwards to `agdi plugins update`.
...
**Output:** `~/.agdi/workspace/memory/YYYY-MM-DD-slug.md`
