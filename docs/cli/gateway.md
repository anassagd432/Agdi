---
summary: "AGDI Gateway CLI (`agdi gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is AGDI’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `agdi gateway …`.
...
- `--token <token>`: token override (also sets `AGDI_GATEWAY_TOKEN` for the process).
- `--password <password>`: password override (also sets `AGDI_GATEWAY_PASSWORD` for the process).
...
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `agdi.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
