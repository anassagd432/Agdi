...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Agdi Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`agdi acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Agdi gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
agdi acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
agdi acp --url wss://gateway-host:18789 --token-file ~/.agdi/gateway.token

# Attach to an existing session key
agdi acp --session agent:main:main

# Attach by label (must already exist)
agdi acp --session-label "support inbox"

# Reset the session key before the first prompt
agdi acp --session agent:main:main --reset-session
```
...
agdi acp client --server-args --url wss://gateway-host:18789 --token-file ~/.agdi/gateway.token

# Override the server command (default: agdi)
agdi acp client --server "node" --server-args agdi.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an Agdi Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `agdi acp` over stdio.

Example config (persisted):

```bash
agdi config set gateway.remote.url wss://gateway-host:18789
agdi config set gateway.remote.token <token>
```
...
```bash
agdi acp --session agent:main:main
agdi acp --session agent:design:main
agdi acp --session agent:qa:bug-123
```
...
Agdi bot over ACP, use `acpx` with its built-in `agdi` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx agdi` at `agdi acp`.
3. Target the Agdi session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Agdi ACP session
acpx agdi exec "Summarize the active Agdi session state."

# Persistent named session for follow-up turns
acpx agdi sessions ensure --name codex-bridge
acpx agdi -s codex-bridge --cwd /path/to/repo \
  "Ask my Agdi work agent for recent context relevant to this repo."
```

If you want `acpx agdi` to target a specific Gateway and session key every
time, override the `agdi` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "agdi": {
      "command": "env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 agdi acp --url ws://127.0.0.1:18789 --token-file ~/.agdi/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Agdi checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 node agdi.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Agdi agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Agdi ACP” to start a thread.
...
- `agdi acp client` sets `OPENCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `agdi`).
