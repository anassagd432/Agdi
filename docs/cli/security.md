---
summary: "CLI reference for `agdi security` (audit and fix common security footguns)"
read_when:
...
agdi security audit --deep --token <token>
...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules), and reminds you that Agdi is a personal-assistant trust model by default.
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `agdi.browserConfigEpoch`) and recommends `agdi sandbox recreate --browser --all`.
...
```bash
agdi security audit --json | jq '.summary'
agdi security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
agdi security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
