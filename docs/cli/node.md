---
summary: "CLI reference for `agdi node` (headless node host)"
read_when:
...
`agdi node run` and `agdi node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):
...
```bash
agdi node install --host <gateway-host> --port 18789
```
...
Use `agdi node run` for a foreground node host (no service).
...
- `agdi approvals --node <id|name|ip>` (edit from the Gateway)
