...
Agdi can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Agdi can send audio.
...
Agdi defaults to Microsoft (unless disabled via
...
`~/.agdi/settings/tts.json`, override with `OPENCLAW_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Agdi retries with MP3.
