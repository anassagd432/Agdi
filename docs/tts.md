...
AGDI can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere AGDI can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, AGDI defaults
...
TTS config lives under `messages.tts` in `agdi.json`.
...
- If `provider` is **unset**, AGDI prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.agdi/settings/tts.json`, override with `AGDI_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, AGDI retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so AGDI registers
