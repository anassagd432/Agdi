...
read_when:
  ...
  - 你需要 MiniMax 设置指南
summary: 在 Agdi 中使用 MiniMax M2.5
title: MiniMax
x-i18n:
...
```bash
agdi plugins enable minimax  # 如果已加载则跳过。
agdi gateway restart  # 如果 gateway 已在运行，则重启
...
详情请参阅 [MiniMax plugin README](https://github.com/agdi/agdi/tree/main/extensions/minimax)。
...
- 使用 `agdi models list` 和 `agdi models set minimax/MiniMax-M2.5` 进行切换。
...
- 运行 `agdi configure` 并选择 **MiniMax M2.5**，或者
