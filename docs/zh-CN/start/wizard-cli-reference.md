...
<Steps>
  ...
  <Step title="渠道">
    ...
    - 私信安全：默认是配对。首次私信会发送一个代码；通过
      `agdi pairing approve <channel> <code>` 批准，或使用 allowlist。
  </Step>
  <Step title="守护进程安装">
    - macOS：LaunchAgent
  ...
  <Step title="健康检查">
    - 启动 Gateway 网关（如有需要），并运行 `agdi health`。
    - `agdi status --deep` 会在状态输出中添加 Gateway 网关健康探测。
  </Step>
  <Step title="Skills">
    - 读取可用的 Skills 并检查要求。
...
- OAuth 凭证：`~/.agdi/credentials/oauth.json`
- Auth profile（API 密钥 + OAuth）：`~/.agdi/agents/<agentId>/agent/auth-profiles.json`
...
`~/.agdi/credentials/oauth.json`（或 `$OPENCLAW_STATE_DIR/credentials/oauth.json`）
到 Gateway 网关主机。
</Note>
...
WhatsApp 凭证位于 `~/.agdi/credentials/whatsapp/<accountId>/`。
...
- 下载适当的发布资源
- 将其存储在 `~/.agdi/tools/signal-cli/<version>/`
