...
CLI 新手引导是在 macOS、
Linux 或 Windows（通过 WSL2；强烈推荐）上设置 Agdi 的**推荐**方式。
...
`agdi dashboard` 并在浏览器中聊天。文档：[Dashboard](/web/dashboard)。
</Info>
...
`agdi configure --section web` 进行配置。文档：[Web 工具](/tools/web)。
</Tip>
...
2. **工作区** —— 智能体文件的位置（默认 `~/.agdi/workspace`）。会植入引导文件。
...
如果配置无效或包含旧版键，新手引导会先要求你运行 `agdi doctor`。
</Note>
...
使用 `agdi agents add <name>` 创建一个单独的智能体，它拥有自己的工作区、
...
- CLI 命令参考：[`agdi onboard`](/cli/onboard)
