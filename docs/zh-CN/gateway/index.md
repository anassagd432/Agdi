...
- 配置热重载监视 `~/.agdi/agdi.json`（或 `OPENCLAW_CONFIG_PATH`）。
...
- 默认在 `canvasHost.port`（默认 `18793`）上启动 Canvas 文件服务器，从 `~/.agdi/workspace/canvas` 提供 `http://<gateway-host>:18793/__agdi__/canvas/`。使用 `canvasHost.enabled=false` 或 `OPENCLAW_SKIP_CANVAS_HOST=1` 禁用。
...
- `OPENCLAW_STATE_DIR=~/.agdi-dev`
- `OPENCLAW_CONFIG_PATH=~/.agdi-dev/agdi.json`
