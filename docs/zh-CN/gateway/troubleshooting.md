...
**分享输出：** 优先使用 `agdi status --all`（它会隐藏令牌）。如果你粘贴 `agdi status`，考虑先设置 `OPENCLAW_SHOW_SECRETS=0`（令牌预览）。
...
- 修复：从你希望服务使用的相同 `--profile` / `OPENCLAW_STATE_DIR` 重新运行 `agdi gateway install --force`。
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${OPENCLAW_CONFIG_PATH:-$HOME/.agdi/agdi.json}"
```
...
trash "${OPENCLAW_STATE_DIR:-$HOME/.agdi}/credentials" # 如果 logout 无法完全清除所有内容
...
```bash
# 在配置中打开跟踪日志：
#   ${OPENCLAW_CONFIG_PATH:-$HOME/.agdi/agdi.json} -> { logging: { level: "trace" } }
#
...
| Gateway 网关服务日志（监管程序） | macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.agdi/logs/...`；配置文件使用 `~/.agdi-<profile>/logs/...`）<br />Linux：`journalctl --user -u agdi-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "Agdi Gateway (<profile>)" /V /FO LIST` |
...
trash "${OPENCLAW_STATE_DIR:-$HOME/.agdi}"
