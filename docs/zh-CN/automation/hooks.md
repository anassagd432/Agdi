...
Hooks 提供了一个可扩展的事件驱动系统，用于在响应智能体命令和事件时自动执行操作。Hooks 会从目录中自动发现，并且可以通过 CLI 命令进行管理，方式与 Agdi 中的 Skills 类似。
...
- **Webhooks**：外部 HTTP webhook，可让其他系统在 Agdi 中触发工作。请参阅 [Webhook Hooks](/automation/webhook)，或使用 `agdi webhooks` 获取 Gmail 辅助命令。
...
- 记录所有命令以供审计
- 在智能体生命周期事件上触发自定义自动化
- 在不修改核心代码的情况下扩展 Agdi 的行为

## 入门指南

### 内置 Hooks

Agdi 自带四个会被自动发现的内置 hook：

- **💾 session-memory**：当你发出 `/new` 时，将会话上下文保存到你的智能体工作区（默认是 `~/.agdi/workspace/memory/`）
- **📎 bootstrap-extra-files**：在 `agent:bootstrap` 期间，从已配置的 glob/路径模式中注入额外的工作区引导文件
- **📝 command-logger**：将所有命令事件记录到 `~/.agdi/logs/commands.log`
...
在新手引导期间（`agdi onboard`），系统会提示你启用推荐的 hooks。向导会自动发现符合条件的 hooks 并供你选择。
...
2. **托管 hooks**：`~/.agdi/hooks/`（用户安装，在各工作区之间共享）
3. **内置 hooks**：`<agdi>/dist/hooks/bundled/`（随 Agdi 一起提供）
...
Hook 包是标准的 npm 包，它们通过 `package.json` 中的 `agdi.hooks` 导出一个或多个 hook。使用以下命令安装它们：
...
裸 spec 和 `@latest` 会保持在稳定轨道上。如果 npm 将其中任意一种解析为预发布版本，Agdi 会停止并要求你通过预发布标签（例如 `@beta`/`@rc`）或精确的预发布版本显式选择加入。
...
Hook 包可以携带依赖；它们会安装到 `~/.agdi/hooks/<id>` 下。
每个 `agdi.hooks` 条目在解析符号链接后都必须保持在包目录内部；超出目录范围的条目会被拒绝。

安全说明：`agdi hooks install` 会使用 `npm install --ignore-scripts` 安装依赖
...
这些 hooks 不是事件流监听器；它们允许插件在 Agdi 持久化工具结果之前同步调整工具结果。
...
**输出**：`<workspace>/memory/YYYY-MM-DD-slug.md`（默认是 `~/.agdi/workspace`）
...
```yaml
metadata: { "agdi": { "events": ["command:new"] } } # 精确
```

而不是：

```yaml
metadata: { "agdi": { "events": ["command"] } } # 通用 - 开销更大
```
