...
所有日志配置都在 `~/.agdi/agdi.json` 的 `logging` 下。
...
- AGDI 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `agdi plugins enable diagnostics-otel` 启用插件。
...
- `agdi.tokens`（计数器，属性：`agdi.token`、`agdi.channel`、`agdi.provider`、`agdi.model`）
- `agdi.cost.usd`（计数器，属性：`agdi.channel`、`agdi.provider`、`agdi.model`）
- `agdi.run.duration_ms`（直方图，属性：`agdi.channel`、`agdi.provider`、`agdi.model`）
- `agdi.context.tokens`（直方图，属性：`agdi.context`、`agdi.channel`、`agdi.provider`、`agdi.model`）

消息流：

- `agdi.webhook.received`（计数器，属性：`agdi.channel`、`agdi.webhook`）
- `agdi.webhook.error`（计数器，属性：`agdi.channel`、`agdi.webhook`）
- `agdi.webhook.duration_ms`（直方图，属性：`agdi.channel`、`agdi.webhook`）
- `agdi.message.queued`（计数器，属性：`agdi.channel`、`agdi.source`）
- `agdi.message.processed`（计数器，属性：`agdi.channel`、`agdi.outcome`）
- `agdi.message.duration_ms`（直方图，属性：`agdi.channel`、`agdi.outcome`）

队列 + 会话：

- `agdi.queue.lane.enqueue`（计数器，属性：`agdi.lane`）
- `agdi.queue.lane.dequeue`（计数器，属性：`agdi.lane`）
- `agdi.queue.depth`（直方图，属性：`agdi.lane` 或 `agdi.channel=heartbeat`）
- `agdi.queue.wait_ms`（直方图，属性：`agdi.lane`）
- `agdi.session.state`（计数器，属性：`agdi.state`、`agdi.reason`）
- `agdi.session.stuck`（计数器，属性：`agdi.state`）
- `agdi.session.stuck_age_ms`（直方图，属性：`agdi.state`）
- `agdi.run.attempt`（计数器，属性：`agdi.attempt`）
...
- `agdi.webhook.error`
  - `agdi.channel`、`agdi.webhook`、`agdi.chatId`、`agdi.error`
- `agdi.message.processed`
  - `agdi.channel`、`agdi.outcome`、`agdi.chatId`、`agdi.messageId`、`agdi.sessionKey`、`agdi.sessionId`、`agdi.reason`
- `agdi.session.stuck`
  - `agdi.state`、`agdi.ageMs`、`agdi.queueDepth`、`agdi.sessionKey`、`agdi.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `agdi doctor`。
