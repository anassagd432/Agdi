...
Agdi 提供三个安装脚本，由 `agdi.ai` 提供。

| 脚本                               | 平台                  | 功能                                                                          |
| ---------------------------------- | --------------------- | ----------------------------------------------------------------------------- |
| [`install.sh`](#installsh)         | macOS / Linux / WSL   | 如有需要则安装 Node，通过 npm（默认）或 git 安装 Agdi，并可运行新手引导。 |
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL   | 将 Node + Agdi 安装到本地前缀（`~/.agdi`）中。无需 root。             |
| [`install.ps1`](#installps1)       | Windows（PowerShell） | 如有需要则安装 Node，通过 npm（默认）或 git 安装 Agdi，并可运行新手引导。 |
...
<Note>
如果安装成功但在新终端中找不到 `agdi`，请参见 [Node.js 故障排除](/install/node#troubleshooting)。
</Note>
...
<Steps>
  ...
  <Step title="默认确保使用 Node.js 24">
    检查 Node 版本，并在需要时安装 Node 24（macOS 上使用 Homebrew，Linux apt/dnf/yum 上使用 NodeSource 设置脚本）。为了兼容性，Agdi 仍支持 Node 22 LTS，目前为 `22.16+`。
  </Step>
  <Step title="确保安装 Git">
    如果缺少 Git，则安装它。
  </Step>
  <Step title="安装 Agdi">
    - `npm` 方法（默认）：全局 npm 安装
    - `git` 方法：克隆/更新仓库，使用 pnpm 安装依赖，构建，然后将包装器安装到 `~/.local/bin/agdi`
  </Step>
  <Step title="安装后任务">
    - 在升级和 git 安装时运行 `agdi doctor --non-interactive`（尽力而为）
...
如果在 Agdi 检出目录中运行（`package.json` + `pnpm-workspace.yaml`），脚本会提供：
...
| `--git-dir <path>`                    | 检出目录（默认：`~/agdi`）。别名：`--dir`     |
...
<Info>
适用于你希望所有内容都放在本地前缀（默认 `~/.agdi`）下，并且不依赖系统 Node 的环境。
</Info>
...
<Steps>
  ...
  <Step title="在前缀下安装 Agdi">
    使用 `--prefix <prefix>` 通过 npm 安装，然后将包装器写入 `<prefix>/bin/agdi`。
  </Step>
</Steps>
...
| 标志                   | 说明                                                                   |
| ---------------------- | ---------------------------------------------------------------------- |
| `--prefix <path>`      | 安装前缀（默认：`~/.agdi`）                                        |
| `--version <ver>`      | Agdi 版本或 dist-tag（默认：`latest`）                             |
| `--node-version <ver>` | Node 版本（默认：`22.22.0`）                                           |
| `--json`               | 输出 NDJSON 事件                                                       |
| `--onboard`            | 安装后运行 `agdi onboard`                                          |
...
| `OPENCLAW_VERSION=<ver>`                    | Agdi 版本或 dist-tag                               |
...
<Steps>
  ...
  <Step title="安装 Agdi">
    ...
    - `git` 方法：克隆/更新仓库，使用 pnpm 安装/构建，并将包装器安装到 `%USERPROFILE%\.local\bin\agdi.cmd`
  </Step>
  <Step title="安装后任务">
    在可能情况下将所需 bin 目录添加到用户 PATH，然后在升级和 git 安装时运行 `agdi doctor --non-interactive`（尽力而为）。
  </Step>
</Steps>
...
| `-GitDir <path>`            | 检出目录（默认：`%USERPROFILE%\agdi`）   |
| `-NoOnboard`                | 跳过新手引导                                 |
