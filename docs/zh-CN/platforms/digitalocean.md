---
read_when:
  - 在 DigitalOcean 上设置 Agdi
  - 寻找适合 Agdi 的低价 VPS 托管
summary: 在 DigitalOcean 上运行 Agdi（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 Agdi

## 目标

在 DigitalOcean 上以 **每月 6 美元**（或预留定价时每月 4 美元）运行一个持久化的 Agdi Gateway 网关。
...
- `~/.agdi/` — 配置、凭证、会话数据
- `~/.agdi/workspace/` — 工作区（`SOUL.md`、memory 等）
