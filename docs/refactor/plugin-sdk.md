...
- Publish as `agdi/plugin-sdk` (or export from core under `agdi/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `AGDIPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AGDIConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AGDIConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AGDIConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AGDIConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AGDIConfig): string;
  };
};
```
...
- Introduce `agdi/plugin-sdk`.
- Add `api.runtime` to `AGDIPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `agdiRuntime: ">=2026.2.0"`).
