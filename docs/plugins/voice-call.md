...
read_when:
  - You want to place an outbound voice call from Agdi
...
Voice calls for Agdi via a plugin. Supports outbound notifications and
...
- Use `agdi voicecall ...` or the `voice_call` tool
...
```bash
agdi plugins install ./extensions/voice-call
...
```bash
agdi voicecall call --to "+15555550123" --message "Hello from Agdi"
agdi voicecall start --to "+15555550123"   # alias for call
...
agdi voicecall tail
agdi voicecall latency                     # summarize turn latency from logs
