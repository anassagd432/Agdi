...
read_when:
  ...
  - You want a reference for all registration methods on AgdiPluginApi
...
The `register(api)` callback receives an `AgdiPluginApi` object with these
methods:
...
| `api.config`             | `AgdiConfig`          | Current config snapshot                                   |
...
<Warning>
  Never import your own plugin through `agdi/plugin-sdk/<your-plugin>`
