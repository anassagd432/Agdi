...
read_when:
  - You are building an Agdi plugin
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Agdi can inspect without loading plugin code.                                      |
...
Agdi reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, Agdi falls back to `choiceId`.                                        |
...
- The manifest is **required for native Agdi plugins**, including local filesystem loads.
