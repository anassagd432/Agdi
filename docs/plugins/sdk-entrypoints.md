...
**Import:** `agdi/plugin-sdk/plugin-entry`
...
| `registerFull` | `(api: AgdiPluginApi) => void`                               | No       | —                   |
...
Agdi loads this instead of the full entry when a channel is disabled,
...
Agdi classifies loaded plugins by their registration behavior:
...
Use `agdi plugins inspect <id>` to see a plugin's shape.
