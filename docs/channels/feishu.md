...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Agdi to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Agdi releases, so no separate plugin install
is required.
...
If you just installed Agdi, run onboarding:
...
2. Configuring app credentials in Agdi
...
1. You already ran `agdi channels add` for Feishu
2. The gateway is running (`agdi gateway status`)
...
```bash
agdi pairing approve feishu <CODE>
```
...
2. Run `agdi logs --follow` and look for `chat_id`
...
2. Run `agdi logs --follow` and look for `open_id`
...
| `agdi gateway restart` | Restart gateway service       |
| `agdi logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `agdi gateway status`
6. Check logs: `agdi logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.agdi/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.agdi/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
