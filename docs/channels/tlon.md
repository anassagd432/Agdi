...
Tlon is a decentralized messenger built on Urbit. AGDI connects to your Urbit ship and can
...
```bash
agdi plugins install ./extensions/tlon
```
...
By default, AGDI blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `agdi message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, AGDI replies in-thread.
