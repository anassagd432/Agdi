...
```bash
agdi plugins install @openclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
agdi plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Agdi" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `agdi pairing list nextcloud-talk`
  - `agdi pairing approve nextcloud-talk <CODE>`
