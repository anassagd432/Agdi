...
Agdi routes replies **back to the channel where a message came from**. The
...
Agdi infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Agdi still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Agdi would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.agdi/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.agdi`):

- `~/.agdi/agents/<agentId>/sessions/sessions.json`
