...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Agdi HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `agdi pairing approve slack <code>`.
...
- The interactive callback values are Agdi-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Agdi falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Agdi is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Agdi",
    "description": "Slack connector for Agdi"
  },
  "features": {
    "bot_user": {
      "display_name": "Agdi",
    ...
    "slash_commands": [
      {
        "command": "/agdi",
        "description": "Send a message to Agdi",
        "should_escape": false
      }
    ]
  },
...
Agdi supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Agdi falls back to normal delivery for remaining payloads.
