...
Matrix is the Matrix channel plugin for Agdi.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Plugin required

Matrix is a plugin and is not bundled with core Agdi.
...
```bash
agdi plugins install ./extensions/matrix
```
...
```bash
agdi channels add
agdi configure --section channels
```
...
- To resolve room names before saving them, use `agdi channels resolve --channel matrix "Project Room"`.
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "Agdi Gateway",
    },
  },
}
```

Matrix stores cached credentials in `~/.agdi/credentials/matrix/`.
...
By default, Matrix messages from other configured Agdi Matrix accounts are ignored.
...
- Agdi still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; Agdi treats "bot-authored" as "sent by another configured Matrix account on this Agdi gateway".
...
```bash
agdi matrix verify status --include-recovery-key --json
```
...
```bash
agdi matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
agdi matrix verify bootstrap --force-reset-cross-signing
```
...
```bash
agdi matrix verify device "<your-recovery-key>" --verbose
```
...
```bash
agdi matrix verify backup restore
```

Verbose restore diagnostics:

```bash
agdi matrix verify backup restore --verbose
```
...
```bash
agdi matrix verify status --account assistant
agdi matrix verify backup restore --account assistant
agdi matrix devices list --account assistant
```
...
Agdi treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `agdi matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for Agdi to treat the device as fully verified.

### What bootstrap does

`agdi matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, Agdi tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
If you intentionally want to discard the current room-key backup and start a new backup baseline for future messages, use `agdi matrix verify backup reset --yes`.
...
If startup finds broken bootstrap state and `channels.matrix.password` is configured, Agdi can attempt a stricter repair path.
If the current device is already owner-signed, Agdi preserves that identity instead of resetting it automatically.

Upgrading from the previous public Matrix plugin:

- Agdi automatically reuses the same Matrix account, access token, and device identity when possible.
- Before any actionable Matrix migration changes run, Agdi creates or reuses a recovery snapshot under `~/Backups/agdi-migrations/`.
- If you use multiple Matrix accounts, set `channels.matrix.defaultAccount` before upgrading from the old flat-store layout so Agdi knows which account should receive that shared legacy state.
- If the previous plugin stored a Matrix room-key backup decryption key locally, startup or `agdi doctor --fix` will import it into the new recovery-key flow automatically.
...
- If the Matrix access token changes later for the same account, homeserver, and user, Agdi now prefers reusing the most complete existing token-hash storage root instead of starting from an empty Matrix state directory.
- On the next gateway start, backed-up room keys are restored automatically into the new crypto store.
- If the old plugin had local-only room keys that were never backed up, Agdi will warn clearly. Those keys cannot be exported automatically from the previous rust crypto store, so some old encrypted history may remain unavailable until recovered manually.
...
`~/.agdi/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/`.
...
When the token changes but the account identity stays the same, Agdi reuses the best existing
...
Agdi currently provides that in Node by:
...
Under Agdi's security model, the gateway host and local Agdi state directory are already inside the trusted operator boundary, so this is primarily an operational durability concern rather than a separate remote trust boundary.

Planned improvement:

- add SecretRef support for persistent Matrix key material so recovery keys and related store-encryption secrets can be sourced from Agdi secrets providers instead of only local files
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by Agdi.
For self-verification flows, Agdi also starts the SAS flow automatically when emoji verification becomes available and confirms its own side.
For verification requests from another Matrix user/device, Agdi auto-accepts the request and then waits for the SAS flow to proceed normally.
...
Agdi does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old Agdi-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
agdi matrix devices list
```

Remove stale Agdi-managed devices with:

```bash
agdi matrix devices prune-stale
```

### Direct Room Repair

If direct-message state gets out of sync, Agdi can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
agdi matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
agdi matrix direct repair --user-id @alice:example.org
```
...
Ack reactions use the standard Agdi resolution order:
...
```bash
agdi pairing list matrix
agdi pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, Agdi reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.
...
Set `defaultAccount` when you want Agdi to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `agdi matrix verify ...` and `agdi matrix devices ...` when you want to override that implicit selection for one command.

## Private/LAN homeservers

By default, Agdi blocks private/internal Matrix homeservers for SSRF protection unless you
...
Matrix accepts these target forms anywhere Agdi asks you for a room or user target:
...
- `autoJoinAllowlist`: rooms/aliases allowed when `autoJoin` is `allowlist`. Alias entries are resolved to room IDs during invite handling; Agdi does not trust alias state claimed by the invited room.
