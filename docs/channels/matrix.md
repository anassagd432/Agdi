...
Matrix is an open, decentralized messaging protocol. AGDI connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `agdi plugins install @agdi/matrix`
   - From a local checkout: `agdi plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: AGDI calls the same
     login endpoint, stores the access token in `~/.agdi/credentials/matrix/credentials.json`,
...
- On first connection, AGDI requests device verification from your other sessions.
...
`~/.agdi/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, AGDI resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
