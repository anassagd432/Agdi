...
read_when:
  - Setting up Zalo Personal for Agdi
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Agdi.
...
- `agdi pairing list zalouser`
- `agdi pairing approve zalouser <code>`
...
- On startup, Agdi resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Agdi stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Agdi state. Example:
...
- Agdi sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Agdi sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `agdi channels logout --channel zalouser && agdi channels login --channel zalouser`
...
- The channel now runs fully in Agdi without external CLI binaries.
