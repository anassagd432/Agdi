...
read_when:
  - You want Agdi to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Agdi to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`agdi onboard`) and `agdi channels add` list optional channel plugins.
...
```bash
agdi plugins install --link <path-to-agdi>/extensions/nostr
```
...
agdi channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "agdi@example.com",
        lud16: "agdi@example.com",
      },
    },
  },
}
