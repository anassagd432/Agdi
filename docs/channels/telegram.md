...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    AGDI can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    For text-only replies, AGDI keeps the same preview message and performs a final edit in place (no second message).
    ...
    `streamMode` is separate from block streaming. When block streaming is explicitly enabled for Telegram, AGDI skips the preview stream to avoid double-streaming.
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `agdi doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `agdi doctor --fix` can resolve legacy `@username` entries to IDs.
