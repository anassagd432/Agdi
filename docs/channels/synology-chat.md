---
summary: "Synology Chat webhook setup and Agdi config"
read_when:
  - Setting up Synology Chat with Agdi
...
```bash
agdi plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `agdi onboard` now shows Synology Chat in the same channel setup list as `agdi channels add`.
   - Non-interactive setup: `agdi channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Agdi gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Agdi.
   - Guided: `agdi onboard`
   - Direct: `agdi channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `agdi pairing list synology-chat`
  - `agdi pairing approve synology-chat <CODE>`
...
```bash
agdi message send --channel synology-chat --target 123456 --text "Hello from Agdi"
agdi message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Agdi now rejects duplicate exact paths
