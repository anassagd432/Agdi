...
Agdi can **summarize inbound media** (image/audio/video) before the reply pipeline runs. It auto‑detects when local tools or provider keys are available, and can be disabled or customized. If understanding is off, models still receive the original files/URLs as usual.

Vendor-specific media behavior is registered by vendor plugins, while Agdi
...
- If `<capability>.enabled: true` but no models are configured, Agdi tries the
  **active reply model** when its provider supports the capability.

### Auto-detect media understanding (default)

If `tools.media.<capability>.enabled` is **not** set to `false` and you haven’t
configured models, Agdi auto-detects in this order and **stops at the first
working option**:
...
When provider-based **audio** and **video** media understanding is enabled, Agdi
...
If the proxy value is malformed, Agdi logs a warning and falls back to direct
fetch.

## Capabilities (optional)

If you set `capabilities`, the entry only runs for those media types. For shared
lists, Agdi can infer defaults:
...
## Provider support matrix (Agdi integrations)
...
- `parakeet-mlx` note: with `--output-dir`, Agdi reads `<output-dir>/<media-basename>.txt` when output format is `txt` (or unspecified); non-`txt` formats fall back to stdout.
