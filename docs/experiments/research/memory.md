...
read_when:
  - Designing workspace memory (~/.agdi/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep AGDI integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.agdi/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.agdi/workspace` as canonical human-readable memory.
...
```
~/.agdi/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in AGDI**, but keep a separable core library.

### Why integrate into AGDI?
...
- You want the agent itself to call the tools:
  - `agdi memory recall "…" --k 25 --since 30d`
  - `agdi memory reflect --since 7d`
...
Pragmatic take for `~/.agdi/workspace`:
