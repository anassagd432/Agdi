...
function normalizeConsoleLevel(level?: string): LogLevel {
  ...
  if (!level && process.env.VITEST === "true" && (process.env.AGDI_TEST_CONSOLE ?? process.env.OPENCLAW_TEST_CONSOLE) !== "1") {
    return "silent";
  }
...
function resolveConsoleSettings(): ConsoleSettings {
  ...
  if (
    ...
    (process.env.AGDI_TEST_CONSOLE ?? process.env.OPENCLAW_TEST_CONSOLE) !== "1" &&
    !isVerbose() &&
    !envLevel &&
