...
export const DEFAULT_LOG_DIR = resolvePreferredAGDITmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "agdi.log"); // legacy single-file path

const LOG_PREFIX = "agdi";
...
function resolveSettings(): ResolvedSettings {
  let cfg: AGDIConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => AGDIConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.AGDI_TEST_FILE_LOG !== "1" ? "silent" : "info";
