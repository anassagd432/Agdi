...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["AGDI_STATE_DIR"]);
  ...
  it("persists runs to disk and resumes after restart", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-subagent-"));
  ...
  it("uses isolated temp state when AGDI_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "agdi-test-state"));
  });
});
