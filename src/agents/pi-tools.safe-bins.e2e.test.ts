...
describe("createAGDICodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    if (process.platform === "win32") {
      return;
    }

    const { createAGDICodingTools } = await import("./pi-tools.js");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-safe-bins-"));
    ...
    const envSnapshot = captureEnv(["AGDI_SHELL_ENV_TIMEOUT_MS"]);
    const result = await (async () => {
      try {
        process.env.AGDI_SHELL_ENV_TIMEOUT_MS = "1000";
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    ...
    const { createAGDICodingTools } = await import("./pi-tools.js");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-safe-bins-expand-"));
