...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const envSnapshot = captureEnv(["AGDI_STATE_DIR", "AGDI_PROFILE"]);
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-test-"));
    process.env.AGDI_STATE_DIR = stateDir;
    process.env.AGDI_PROFILE = "isolated";

    try {
      const tool = createAGDITools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: agdi --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/agdi" } }\n}\n';
