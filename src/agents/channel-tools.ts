...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as AGDIConfig);
...
export function listChannelAgentTools(params: { cfg?: AGDIConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: AGDIConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {
