...
describe("ensureAuthProfileStore", () => {
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-auth-merge-"));
    const previousAgentDir = process.env.AGDI_AGENT_DIR;
    ...
    try {
      ...
      process.env.AGDI_AGENT_DIR = mainDir;
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.AGDI_AGENT_DIR = previousAgentDir;
      }
