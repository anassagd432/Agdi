...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses AGDI_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      AGDI_HOME: "/srv/agdi-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/agdi-home"), ".agdi", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".agdi", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("agdi-workspace-");
