...
function resolveExecConfig(params: { cfg?: AGDIConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: AGDIConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createAGDICodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createAGDIReadTool(freshReadTool);
