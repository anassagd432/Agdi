...
export function resolveOpenClawAgentDir(env: NodeJS.ProcessEnv = process.env): string {
  const override = env.AGDI_AGENT_DIR?.trim() || env.OPENCLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
  if (override) {
...
export function ensureOpenClawAgentEnv(): string {
  ...
  if (!process.env.AGDI_AGENT_DIR && !process.env.OPENCLAW_AGENT_DIR) {
