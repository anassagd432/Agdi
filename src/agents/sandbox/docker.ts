...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "agdi.sandbox=1");
  args.push("--label", `agdi.sessionKey=${params.scopeKey}`);
  args.push("--label", `agdi.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `agdi.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "agdi.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`agdi sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`agdi sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("agdi sandbox recreate --all");
}
