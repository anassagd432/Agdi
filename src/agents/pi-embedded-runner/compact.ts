...
import { resolveAGDIDocsPath } from "../docs-path.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveAGDIAgentDir();
  await ensureAGDIModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createAGDICodingTools({
      exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveAGDIDocsPath({
      workspaceDir: effectiveWorkspace,
