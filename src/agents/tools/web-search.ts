...
type WebSearchConfig = NonNullable<AGDIConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: AGDIConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("agdi configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.agdi.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://agdi.ai",
      "X-Title": "AGDI Web Search",
    },
