...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("agdi-ws-", async (workspaceDir) => {
      await withTempDir("agdi-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createAGDICodingTools({ workspaceDir });
    ...
    await withTempDir("agdi-ws-", async (workspaceDir) => {
      await withTempDir("agdi-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello agdi");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("agdi-ws-", async (workspaceDir) => {
      await withTempDir("agdi-override-", async (overrideDir) => {
        const tools = createAGDICodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("agdi-sandbox-", async (sandboxDir) => {
      await withTempDir("agdi-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createAGDICodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
