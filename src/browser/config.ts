import type { BrowserConfig, BrowserProfileConfig, AGDIConfig } from "../config/config.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "agdi" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_AGDI_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "agdi" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_AGDI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_AGDI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an AGDI browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for agdi/agdi CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AGDIConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_AGDI_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "agdi";
