...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/agdi" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "agdi",
        profiles: {
          agdi: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchAGDIChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveAGDIUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopAGDIChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.AGDI_GATEWAY_PORT;
    process.env.AGDI_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.AGDI_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.AGDI_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.AGDI_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.AGDI_GATEWAY_TOKEN;
    } else {
      process.env.AGDI_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.AGDI_GATEWAY_PASSWORD;
    } else {
      process.env.AGDI_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
