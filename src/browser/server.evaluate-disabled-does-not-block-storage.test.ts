  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "agdi",
        profiles: {
          agdi: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.AGDI_GATEWAY_PORT;
    process.env.AGDI_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.AGDI_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.AGDI_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.AGDI_GATEWAY_PORT = prevGatewayPort;
    }
    if (prevGatewayToken === undefined) {
      delete process.env.AGDI_GATEWAY_TOKEN;
    } else {
      process.env.AGDI_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.AGDI_GATEWAY_PASSWORD;
    } else {
      process.env.AGDI_GATEWAY_PASSWORD = prevGatewayPassword;
    }
