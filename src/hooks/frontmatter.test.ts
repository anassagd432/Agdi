...
`;
    ...
    expect(parsed.agdi.emoji).toBe("💾");
    expect(parsed.agdi.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.agdi.emoji).toBe("📝");
    expect(parsed.agdi.events).toEqual(["command"]);
    expect(parsed.agdi.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.agdi.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"agdi": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"agdi": {"events": ["test"]}}');
  });
...
describe("resolveAGDIMetadata", () => {
  it("extracts agdi metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAGDIMetadata(frontmatter);
  ...
  it("returns undefined when agdi key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        agdi: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with AGDI" },
            { id: "npm", kind: "npm", package: "@agdi/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@agdi/hook");
  });
...
`;
    ...
    const agdi = resolveAGDIMetadata(frontmatter);
    expect(agdi).toBeDefined();
    expect(agdi?.emoji).toBe("💾");
    expect(agdi?.events).toEqual(["command:new"]);
    expect(agdi?.requires?.config).toEqual(["workspace.dir"]);
    expect(agdi?.install?.[0].kind).toBe("bundled");
  });
