...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.AGDI_BUNDLED_HOOKS_DIR;
    process.env.AGDI_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.AGDI_BUNDLED_HOOKS_DIR;
    } else {
      process.env.AGDI_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
