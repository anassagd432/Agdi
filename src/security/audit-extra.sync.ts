...
function summarizeGroupPolicy(cfg: AGDIConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: AGDIConfig): boolean {
...
function collectModels(cfg: AGDIConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: AGDIConfig): Set<string> {
  const baseCfg: AGDIConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: AGDIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: AGDIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: AGDIConfig): boolean {
...
function isBrowserEnabled(cfg: AGDIConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: AGDIConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: AGDIConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep AGDI_STATE_DIR on a local-only volume and re-run "${formatCliCommand("agdi security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: AGDIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
...
export function collectHooksHardeningFindings(
  ...
  const agdiGatewayToken =
    typeof env.AGDI_GATEWAY_TOKEN === "string" && env.AGDI_GATEWAY_TOKEN.trim()
      ? env.AGDI_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-agdi-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: AGDIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: AGDIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: AGDIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: AGDIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: AGDIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: AGDIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
