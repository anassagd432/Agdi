...
function listAgents(cfg: AGDIConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: AGDIConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AGDIConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<AGDIConfig, EvaluatedBindingsCache>();
