...
const PLUGIN_SDK_SUBPATH_PATTERN = /agdi\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references agdi/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
