...
const shouldWarnCompatImport =
  ...
  (process.env.AGDI_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING ?? process.env.OPENCLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING) !== "1";

if (shouldWarnCompatImport) {
  process.emitWarning(
    "agdi/plugin-sdk/compat is deprecated for new plugins. Migrate to focused agdi/plugin-sdk/<subpath> imports. See https://docs.agdi.ai/plugins/sdk-migration",
    {
      code: "OPENCLAW_PLUGIN_SDK_COMPAT_DEPRECATED",
      detail:
        "Bundled plugins must use scoped plugin-sdk subpaths. External plugins may keep compat temporarily while migrating. Migration guide: https://docs.agdi.ai/plugins/sdk-migration",
    },
  );
}
