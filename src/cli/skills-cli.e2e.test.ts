...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["AGDI_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-bundled-skills-test-"));
    process.env.AGDI_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
