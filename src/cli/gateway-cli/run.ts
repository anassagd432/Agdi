...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile =
    (process.env.AGDI_PROFILE ?? process.env.OPENCLAW_PROFILE)?.trim().toLowerCase() === "dev";
  ...
  if (opts.claudeCliLogs) {
    ...
    process.env.AGDI_CLAUDE_CLI_LOG_OUTPUT = "1";
  ...
  if (opts.rawStream) {
    process.env.AGDI_RAW_STREAM = "1";
    process.env.OPENCLAW_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.AGDI_RAW_STREAM_PATH = rawStreamPath;
  ...
  if ((process.env.AGDI_SERVICE_MARKER ?? process.env.OPENCLAW_SERVICE_MARKER)?.trim()) {
  ...
  if (!opts.allowUnconfigured && mode !== "local") {
    if (!configExists) {
      defaultRuntime.error(
        `Missing config. Run \`${formatCliCommand("agdi setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
      );
    } else {
      defaultRuntime.error(
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or AGDI_GATEWAY_PASSWORD; compatibility: OPENCLAW_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (
    ...
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or AGDI_GATEWAY_TOKEN/AGDI_GATEWAY_PASSWORD; compatibility: OPENCLAW_* also works) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  try {
    ...
    if (
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("agdi gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: AGDI_GATEWAY_TOKEN env if set; compatibility: OPENCLAW_GATEWAY_TOKEN)",
    )
