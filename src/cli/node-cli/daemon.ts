...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("agdi node install"), formatCliCommand("agdi node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("agdi node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    AGDI_LOG_PREFIX: baseEnv.AGDI_LOG_PREFIX ?? "node",
