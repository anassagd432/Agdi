...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    AGDI_STATE_DIR: process.env.AGDI_STATE_DIR,
    AGDI_CONFIG_PATH: process.env.AGDI_CONFIG_PATH,
    AGDI_GATEWAY_PORT: process.env.AGDI_GATEWAY_PORT,
    AGDI_PROFILE: process.env.AGDI_PROFILE,
  };

  beforeEach(() => {
    process.env.AGDI_STATE_DIR = "/tmp/agdi-cli-state";
    process.env.AGDI_CONFIG_PATH = "/tmp/agdi-cli-state/agdi.json";
  ...
  afterEach(() => {
    if (originalEnv.AGDI_STATE_DIR !== undefined) {
      process.env.AGDI_STATE_DIR = originalEnv.AGDI_STATE_DIR;
    } else {
      delete process.env.AGDI_STATE_DIR;
    }

    if (originalEnv.AGDI_CONFIG_PATH !== undefined) {
      process.env.AGDI_CONFIG_PATH = originalEnv.AGDI_CONFIG_PATH;
    } else {
      delete process.env.AGDI_CONFIG_PATH;
    }

    if (originalEnv.AGDI_GATEWAY_PORT !== undefined) {
      process.env.AGDI_GATEWAY_PORT = originalEnv.AGDI_GATEWAY_PORT;
    } else {
      delete process.env.AGDI_GATEWAY_PORT;
    }

    if (originalEnv.AGDI_PROFILE !== undefined) {
      process.env.AGDI_PROFILE = originalEnv.AGDI_PROFILE;
    } else {
      delete process.env.AGDI_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        AGDI_PROFILE: "dev",
        AGDI_STATE_DIR: "/tmp/agdi-daemon-state",
        AGDI_CONFIG_PATH: "/tmp/agdi-daemon-state/agdi.json",
