...
const SANDBOX_EXAMPLES = {
  main: [
    ["agdi sandbox list", "List all sandbox containers."],
    ["agdi sandbox list --browser", "List only browser containers."],
    ["agdi sandbox recreate --all", "Recreate all containers."],
    ["agdi sandbox recreate --session main", "Recreate a specific session."],
    ["agdi sandbox recreate --agent mybot", "Recreate agent containers."],
    ["agdi sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["agdi sandbox list", "List all sandbox containers."],
    ["agdi sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["agdi sandbox recreate --all", "Recreate all containers."],
    ["agdi sandbox recreate --session main", "Recreate a specific session."],
    ["agdi sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["agdi sandbox recreate --browser --all", "Recreate only browser containers."],
    ["agdi sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["agdi sandbox explain", "Show effective sandbox config."],
    ["agdi sandbox explain --session agent:main:main", "Explain a specific session."],
    ["agdi sandbox explain --agent work", "Explain an agent sandbox."],
    ["agdi sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.agdi.ai/cli/sandbox")}\n`,
    )
    .action(() => {
