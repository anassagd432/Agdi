...
async function assertUnmanagedGatewayRestartEnabled(port: number): Promise<void> {
  ...
  const probe = await probeGateway({
    ...
    auth: {
      token: (process.env.AGDI_GATEWAY_TOKEN ?? process.env.OPENCLAW_GATEWAY_TOKEN)?.trim() || undefined,
      password: (process.env.AGDI_GATEWAY_PASSWORD ?? process.env.OPENCLAW_GATEWAY_PASSWORD)?.trim() || undefined,
    },
    timeoutMs: 1_000,
  }).catch(() => null);
