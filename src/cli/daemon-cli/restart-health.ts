...
async function confirmGatewayReachable(port: number): Promise<boolean> {
  const token = (process.env.AGDI_GATEWAY_TOKEN ?? process.env.OPENCLAW_GATEWAY_TOKEN)?.trim() || undefined;
  const password = (process.env.AGDI_GATEWAY_PASSWORD ?? process.env.OPENCLAW_GATEWAY_PASSWORD)?.trim() || undefined;
