...
describe("memory cli", () => {
  ...
  it("documents memory help examples", () => {
    ...
    expect(helpText).toContain("agdi memory status --deep");
    expect(helpText).toContain("Probe embedding provider readiness.");
    expect(helpText).toContain('agdi memory search "meeting notes"');
    expect(helpText).toContain("Quick search using positional query.");
    expect(helpText).toContain('agdi memory search --query "deployment" --max-results 20');
