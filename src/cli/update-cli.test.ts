...
const { resolveAGDIPackageRoot } = await import("../infra/openclaw-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-update-tests-"));
  });
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("agdi-update");
    setTty(false);
    readPackageVersion.mockResolvedValue("2.0.0");

    vi.mocked(resolveAGDIPackageRoot).mockResolvedValue(tempDir);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveAGDIPackageRoot).mockReset();
    ...
    vi.mocked(resolveAGDIPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("agdi");
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("agdi-update-wizard");
    const envSnapshot = captureEnv(["AGDI_GIT_DIR"]);
    try {
      setTty(true);
      process.env.AGDI_GIT_DIR = tempDir;
