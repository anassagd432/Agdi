    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["agdi nodes status", "List known nodes with live status."],
          ["agdi nodes pairing pending", "Show pending node pairing requests."],
          ['agdi nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["agdi nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.agdi.ai/cli/nodes")}\n`,
    );
