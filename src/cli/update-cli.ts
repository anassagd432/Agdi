...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["agdi update", "Update a source checkout (git)"],
        ["agdi update --channel beta", "Switch to beta channel (git + npm)"],
        ["agdi update --channel dev", "Switch to dev channel (git + npm)"],
        ["agdi update --tag beta", "One-off update to a dist-tag or version"],
        ["agdi update --tag main", "One-off package install from GitHub main"],
        ["agdi update --dry-run", "Preview actions without changing anything"],
        ["agdi update --no-restart", "Update without restarting the service"],
        ["agdi update --json", "Output result as JSON"],
        ["agdi update --yes", "Non-interactive (accept downgrade prompts)"],
        ["agdi update wizard", "Interactive update wizard"],
        ["agdi --update", "Shorthand for agdi update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run agdi update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.agdi.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["agdi update status", "Show channel + version status."],
          ["agdi update status --json", "JSON output."],
          ["agdi update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.agdi.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
