...
export function applyCliProfileEnv(params: {
  ...
  const stateDir = env.AGDI_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.AGDI_STATE_DIR?.trim()) {
    env.AGDI_STATE_DIR = stateDir;
  }

  if (!env.AGDI_CONFIG_PATH?.trim()) {
    env.AGDI_CONFIG_PATH = path.join(stateDir, "agdi.json");
  }

  if (profile === "dev" && !env.AGDI_GATEWAY_PORT?.trim()) {
