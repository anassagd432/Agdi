...
export function registerBackupCommand(program: Command) {
  const backup = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "docs.agdi.ai/cli/backup")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["agdi backup create", "Create a timestamped backup in the current directory."],
          [
          ...
          ["agdi backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          [
            "agdi backup verify ~/Backups/latest.tar.gz --json",
