...
${formatHelpExamples([
  ['agdi agent --to +15555550123 --message "status update"', "Start a new session."],
  ['agdi agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'agdi agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'agdi agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['agdi agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.agdi.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.agdi.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['agdi agents set-identity --agent main --name "AGDI" --emoji "🦞"', "Set name + emoji."],
  ["agdi agents set-identity --agent main --avatar avatars/agdi.png", "Set avatar path."],
  [
    "agdi agents set-identity --workspace ~/.agdi/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "agdi agents set-identity --identity-file ~/.agdi/workspace/IDENTITY.md --agent main",
