    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["agdi memory status", "Show index and provider status."],
          ["agdi memory status --deep", "Probe embedding provider readiness."],
          ["agdi memory index --force", "Force a full reindex."],
          ['agdi memory search "meeting notes"', "Quick search using positional query."],
          [
            'agdi memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["agdi memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.agdi.ai/cli/memory")}\n`,
    );
