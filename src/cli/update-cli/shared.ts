...
const AGDI_REPO_URL = "https://github.com/agdi/agdi.git";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("agdi@")) {
    return trimmed.slice("agdi@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.AGDI_GIT_DIR?.trim();
  if (override) {
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AGDI_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `AGDI_GIT_DIR points at a non-git directory: ${params.dir}. Set AGDI_GIT_DIR to an empty folder or an agdi checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`AGDI_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "agdi.mjs");
