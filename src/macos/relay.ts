...
async function main() {
  ...
  const { ensureAGDICliOnPath } = await import("../infra/path-env.js");
  ensureAGDICliOnPath();
  ...
  process.on("uncaughtException", (error) => {
    console.error("[agdi] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[agdi] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
