...
const BUNDLED_VERSION =
  (typeof __AGDI_VERSION__ === "string" && __AGDI_VERSION__) ||
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `agdi --version` behavior for Swift env/version checks.
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (AGDI_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
