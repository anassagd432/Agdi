...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { AGDI_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { AGDI_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports AGDI_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { AGDI_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { AGDI_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { AGDI_SMOKE: "nope" })).toBe("qr");
  });
});
