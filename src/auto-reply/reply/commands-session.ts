...
import { scheduleGatewaySigusr1Restart, triggerAGDIRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting AGDI in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAGDIRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting AGDI via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
