...
type AgentConfig = Partial<NonNullable<NonNullable<AGDIConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.agdi/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
const formatVoiceModeLine = (config?: AGDIConfig, sessionEntry?: SessionEntry): string | null => {
  if (!config) {
    return null;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🔷 AGDI ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: AGDIConfig): string {
  const lines = ["ℹ️ Help", ""];
