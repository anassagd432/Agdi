...
export function isCommandEnabled(cfg: AGDIConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AGDIConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AGDIConfig),
...
export function getCommandDetection(_cfg?: AGDIConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AGDIConfig) {
