...
describe("shared/frontmatter", () => {
  ...
  test("resolveAGDIManifestBlock parses JSON5 metadata and picks agdi block", () => {
    const frontmatter = {
      metadata: "{ agdi: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveAGDIManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });
