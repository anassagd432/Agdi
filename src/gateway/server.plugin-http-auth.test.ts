...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.AGDI_CONFIG_PATH;
  const prevDisableCache = process.env.AGDI_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "agdi-plugin-http-auth-test-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.AGDI_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.AGDI_DISABLE_CONFIG_CACHE;
    } else {
      process.env.AGDI_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
