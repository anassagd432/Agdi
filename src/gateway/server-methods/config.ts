import type { AGDIConfig } from "../../config/types.autonomous.js";
...
import { loadAGDIPlugins } from "../../plugins/loader.js";
import {
  ErrorCodes,
  errorShape,
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: AGDIConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadAGDIPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    logger: {
  ...
  // our cache. However, both loadConfig() and loadAGDIPlugins() already cache
