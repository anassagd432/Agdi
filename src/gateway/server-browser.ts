...
export async function startBrowserControlServerIfEnabled(): Promise<BrowserControlServer | null> {
  if (isTruthyEnvValue(process.env.AGDI_SKIP_BROWSER_CONTROL_SERVER ?? process.env.OPENCLAW_SKIP_BROWSER_CONTROL_SERVER)) {
    return null;
  }
  // Lazy import: keeps startup fast, but still bundles for the embedded
  // gateway (bun --compile) via the static specifier path.
  const override = (process.env.AGDI_BROWSER_CONTROL_MODULE ?? process.env.OPENCLAW_BROWSER_CONTROL_MODULE)?.trim();
