...
describe("handleControlUiHttpRequest", () => {
  it("sets security headers for Control UI responses", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-ui-"));
    try {
  ...
  it("serves bootstrap config JSON under basePath", async () => {
    ...
    try {
      ...
      const { res, end } = makeResponse();
      const handled = handleControlUiHttpRequest(
        { url: `/agdi${CONTROL_UI_BOOTSTRAP_CONFIG_PATH}`, method: "GET" } as IncomingMessage,
        res,
        {
          basePath: "/agdi",
          root: { kind: "resolved", path: tmp },
      ...
      expect(parsed.basePath).toBe("/agdi");
      expect(parsed.assistantName).toBe("Ops");
      expect(parsed.assistantAvatar).toBe("/agdi/avatar/main");
      expect(parsed.assistantAgentId).toBe("main");
    } finally {
      await fs.rm(tmp, { recursive: true, force: true });
    }
