...
beforeAll(async () => {
  envSnapshot = captureEnv(["AGDI_GATEWAY_PORT", "AGDI_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.AGDI_GATEWAY_PORT = String(gatewayPort);
  process.env.AGDI_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createAGDITools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    if (!configPath) {
      throw new Error("AGDI_CONFIG_PATH missing in gateway test environment");
    }
