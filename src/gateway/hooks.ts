...
export function resolveHooksConfig(cfg: AGDIConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AGDIConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-agdi-token"] === "string" ? req.headers["x-agdi-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
