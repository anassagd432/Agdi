...
import { resolveFounderRoutineSessionRequestKey } from "../founder-ops/control-plane.js";
...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = (process.env.AGDI_SKIP_CRON ?? process.env.OPENCLAW_SKIP_CRON) !== "1" && params.cfg.cron?.enabled !== false;
  ...
  const cron = new CronService({
    ...
    runIsolatedAgentJob: async ({ job, message, abortSignal }) => {
      ...
      const sessionKey =
        resolveFounderRoutineSessionRequestKey({
          id: job.id,
          name: job.name,
