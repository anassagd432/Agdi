 ...
 * This prevents literal placeholder strings like `${AGDI_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.AGDI_GATEWAY_TOKEN ?? env.OPENCLAW_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.AGDI_GATEWAY_PASSWORD ?? env.OPENCLAW_GATEWAY_PASSWORD);
}
