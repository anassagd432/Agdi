...
const connectNodeClient = async (params: {
  ...
  const token = process.env.AGDI_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("AGDI_GATEWAY_TOKEN is required for node test clients");
  }
  let settled = false;
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".agdi", "restart-sentinel.json");
