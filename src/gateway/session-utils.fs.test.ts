...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  beforeAll(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-session-fs-test-"));
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __agdi?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker.__agdi?.kind).toBe("compaction");
    expect(marker.__agdi?.id).toBe("comp-1");
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  beforeAll(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-session-preview-test-"));
    storePath = path.join(tmpDir, "sessions.json");
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses AGDI_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/agdi-home"), ".agdi", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("resolveSessionTranscriptCandidates safety", () => {
  test("keeps cross-agent absolute sessionFile when storePath agent context differs", () => {
    const storePath = "/tmp/agdi/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/agdi/agents/ops/sessions/sess-safe.jsonl";
  ...
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/agdi/agents/main/sessions/sessions.json",
    );
...
describe("archiveSessionTranscripts", () => {
  ...
  beforeAll(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-archive-test-"));
    storePath = path.join(tmpDir, "sessions.json");
    vi.stubEnv("AGDI_HOME", tmpDir);
  });
