...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.agdi.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", AGDI_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    const env: Record<string, string | undefined> = {
      HOME: "/Users/test",
      AGDI_PROFILE: "default",
    };
    const repair = await repairLaunchAgentBootstrap({ env });
    expect(repair.ok).toBe(true);

    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const label = "ai.agdi.gateway";
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when AGDI_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.agdi.gateway.plist",
    );
  });

  it("uses profile-specific label when AGDI_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", AGDI_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.agdi.jbphoenix.plist",
    );
  });

  it("prefers AGDI_LAUNCHD_LABEL over AGDI_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      AGDI_PROFILE: "jbphoenix",
      AGDI_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from AGDI_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      AGDI_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty AGDI_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      AGDI_PROFILE: "myprofile",
      AGDI_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.agdi.myprofile.plist",
    );
  });
});
