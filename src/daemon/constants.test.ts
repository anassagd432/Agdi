...
describe("resolveGatewayServiceDescription", () => {
  it("prefers explicit description override", () => {
    expect(
      resolveGatewayServiceDescription({
        env: {
          AGDI_PROFILE: "agdi-work",
          AGDI_SERVICE_VERSION: "2.0.0",
  ...
  it("resolves version from explicit environment map", () => {
    expect(
      resolveGatewayServiceDescription({
        env: {
          AGDI_PROFILE: "work",
          AGDI_SERVICE_VERSION: "local",
          OPENCLAW_PROFILE: "legacy",
          OPENCLAW_SERVICE_VERSION: "old-local",
        },
        environment: { AGDI_SERVICE_VERSION: "remote", OPENCLAW_SERVICE_VERSION: "old-remote" },
      }),
