...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = (args?.env?.AGDI_LAUNCHD_LABEL ?? args?.env?.OPENCLAW_LAUNCHD_LABEL)?.trim();
  if (envLabel) {
    return envLabel;
  }
  return resolveGatewayLaunchAgentLabel(args?.env?.AGDI_PROFILE ?? args?.env?.OPENCLAW_PROFILE);
}
...
export function resolveGatewayLogPaths(env: GatewayServiceEnv): {
  ...
  const prefix = (env.AGDI_LOG_PREFIX ?? env.OPENCLAW_LOG_PREFIX)?.trim() || "gateway";
  return {
    logDir,
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromEnv = parseStrictPositiveInteger(env.AGDI_GATEWAY_PORT ?? env.OPENCLAW_GATEWAY_PORT ?? "");
  return fromEnv ?? null;
}
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.agdi.ai/gateway",
    ].join("\n"),
  );
}
...
export async function findLegacyLaunchAgents(env: GatewayServiceEnv): Promise<LegacyLaunchAgent[]> {
  ...
  for (const label of resolveLegacyGatewayLaunchAgentLabels(env.AGDI_PROFILE ?? env.OPENCLAW_PROFILE)) {
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.AGDI_PROFILE ?? env.OPENCLAW_PROFILE)) {
