...
export function resolveTaskScriptPath(env: GatewayServiceEnv): string {
  const override = (env.AGDI_TASK_SCRIPT ?? env.OPENCLAW_TASK_SCRIPT)?.trim();
  if (override) {
    return override;
  }
  const scriptName = (env.AGDI_TASK_SCRIPT_NAME ?? env.OPENCLAW_TASK_SCRIPT_NAME)?.trim() || "gateway.cmd";
...
function resolveConfiguredGatewayPort(env: GatewayServiceEnv): number | null {
  const raw = (env.AGDI_GATEWAY_PORT ?? env.OPENCLAW_GATEWAY_PORT)?.trim();
  if (!raw) {
    return null;
  }
...
async function resolveScheduledTaskPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  return (
    ...
    parsePositivePort(command?.environment?.AGDI_GATEWAY_PORT ?? command?.environment?.OPENCLAW_GATEWAY_PORT) ??
...
async function activateScheduledTask(params: {
  ...
  const taskDescription = params.description ?? "Agdi Gateway";
