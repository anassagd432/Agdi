...
import type { AGDIConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareAGDIVersions } from "./version.js";
...
function coerceConfig(value: unknown): AGDIConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AGDIConfig;
}
...
function stampConfigVersion(cfg: AGDIConfig): AGDIConfig {
...
function warnIfConfigFromFuture(cfg: AGDIConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareAGDIVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer AGDI (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as AGDIConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AGDIConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AGDIConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AGDIConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as AGDIConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.AGDI_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.AGDI_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.AGDI_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.AGDI_WATCH_SESSION === "string" &&
        deps.env.AGDI_WATCH_SESSION.trim().length > 0
          ? deps.env.AGDI_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.AGDI_WATCH_COMMAND === "string" &&
        deps.env.AGDI_WATCH_COMMAND.trim().length > 0
          ? deps.env.AGDI_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `AGDI_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AGDI_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.AGDI_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): AGDIConfig {
