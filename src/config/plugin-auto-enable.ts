import type { AGDIConfig } from "./config.js";
...
function resolveChannelConfig(cfg: AGDIConfig, channelId: string): Record<string, unknown> | null {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  const entry = channels?.[channelId];
  return isRecord(entry) ? entry : null;
}

function isTelegramConfigured(cfg: AGDIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: AGDIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isIrcConfigured(cfg: AGDIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: AGDIConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: AGDIConfig): boolean {
...
function isIMessageConfigured(cfg: AGDIConfig): boolean {
...
function isWhatsAppConfigured(cfg: AGDIConfig): boolean {
...
function isGenericChannelConfigured(cfg: AGDIConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: AGDIConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AGDIConfig, providerId: string): boolean {
...
function resolveConfiguredPlugins(cfg: AGDIConfig, env: NodeJS.ProcessEnv): PluginEnableChange[] {
...
function isPluginExplicitlyDisabled(cfg: AGDIConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: AGDIConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: AGDIConfig, pluginId: string): AGDIConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
