...
// Keep integration tests deterministic: never read a real agdi.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.AGDI_SESSION_CACHE_TTL_MS;
    process.env.AGDI_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.AGDI_SESSION_CACHE_TTL_MS;
    } else {
      process.env.AGDI_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
