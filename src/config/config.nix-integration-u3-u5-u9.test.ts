...
describe("Nix integration (U3, U5, U9)", () => {
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    it("STATE_DIR defaults to ~/.agdi when env not set", () => {
      expect(resolveStateDir(envWith({ OPENCLAW_STATE_DIR: undefined }))).toMatch(/\.agdi$/);
    });
    ...
    it("CONFIG_PATH defaults to OPENCLAW_HOME/.agdi/agdi.json", () => {
      const customHome = path.join(path.sep, "custom", "home");
      expect(
        resolveConfigPathCandidate(
          envWith({
    ...
    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", () => {
      ...
      ).toBe(path.join(path.resolve("/custom/state"), "agdi.json"));
    });
  });
