...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-agdi",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-agdi#general");
  });
  ...
  it("derives session transcripts dir from AGDI_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { AGDI_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.AGDI_STATE_DIR = "/custom/state";
    try {
