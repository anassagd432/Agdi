...
import type { AGDIConfig, ConfigValidationIssue } from "./types.js";
...
import { AGDISchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: AGDIConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: AGDIConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AGDISchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AGDIConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AGDIConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as AGDIConfig,
  };
}
