...
describe("gateway port resolution", () => {
  it("prefers numeric env values over config", () => {
    expect(resolveGatewayPort({ gateway: { port: 19002 } }, envWith({ AGDI_GATEWAY_PORT: "19001" }))).toBe(
      19001,
    );
  });
  ...
  it("accepts Compose-style IPv4 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ AGDI_GATEWAY_PORT: "127.0.0.1:18789" }),
      ),
    ).toBe(18789);
  });
  ...
  it("accepts Compose-style IPv6 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ AGDI_GATEWAY_PORT: "[::1]:28789" }),
      ),
    ).toBe(28789);
  });
  ...
  it("falls back to config when the Compose-style suffix is invalid", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19003 } },
        envWith({ AGDI_GATEWAY_PORT: "127.0.0.1:not-a-port" }),
      ),
    ).toBe(19003);
  });
  ...
  it("falls back when malformed IPv6 inputs do not provide an explicit port", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19003 } }, envWith({ AGDI_GATEWAY_PORT: "::1" })),
    ).toBe(19003);
    expect(resolveGatewayPort({}, envWith({ AGDI_GATEWAY_PORT: "2001:db8::1" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });

  it("falls back to the default port when env is invalid and config is unset", () => {
    expect(resolveGatewayPort({}, envWith({ AGDI_GATEWAY_PORT: "127.0.0.1:not-a-port" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });
});
...
describe("state + config path candidates", () => {
  function expectAgdiHomeDefaults(env: NodeJS.ProcessEnv): void {
    const configuredHome = env.AGDI_HOME;
    if (!configuredHome) {
      throw new Error("AGDI_HOME must be set for this assertion helper");
    }
  ...
  it("uses AGDI_HOME for default state/config locations", () => {
    ...
    expectAgdiHomeDefaults(env);
  });

  it("still honors OPENCLAW_HOME for compatibility", () => {
    const env = {
      OPENCLAW_HOME: "/srv/openclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv;
    expect(resolveStateDir(env)).toBe(path.join(path.resolve("/srv/openclaw-home"), ".agdi"));
  });
  ...
  it("orders default config candidates in a stable order", () => {
    ...
    const expected = [
      ...
      path.join(resolvedHome, ".agdi", "openclaw.json"),
      path.join(resolvedHome, ".agdi", "clawdbot.json"),
      path.join(resolvedHome, ".agdi", "moldbot.json"),
      path.join(resolvedHome, ".openclaw", "agdi.json"),
  ...
  it("prefers ~/.openclaw when it exists and legacy dir is missing", async () => {
    await withTempDir({ prefix: "agdi-state-" }, async (root) => {
  ...
  it("falls back to existing legacy state dir when ~/.openclaw is missing", async () => {
    await withTempDir({ prefix: "agdi-state-legacy-" }, async (root) => {
      const legacyDir = path.join(root, ".clawdbot");
      await fs.mkdir(legacyDir, { recursive: true });
      const resolved = resolveStateDir({} as NodeJS.ProcessEnv, () => root);
      expect(resolved).toBe(path.join(root, ".agdi"));
    });
  });

  it("CONFIG_PATH prefers existing config when present", async () => {
    await withTempDir({ prefix: "agdi-config-" }, async (root) => {
  ...
  it("respects state dir overrides when config is missing", async () => {
    await withTempDir({ prefix: "agdi-config-override-" }, async (root) => {
  ...
  it("still accepts OPENCLAW_GATEWAY_PORT for compatibility", () => {
    expect(
