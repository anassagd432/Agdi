  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.agdi/logs/agdi.log" },
        hooks: {
          path: "~/.agdi/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".agdi", "logs", "agdi.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".agdi", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".agdi", "telegram.token"),
      );
