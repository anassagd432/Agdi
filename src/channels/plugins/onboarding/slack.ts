...
function setSlackDmPolicy(cfg: AGDIConfig, dmPolicy: DmPolicy) {
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "AGDI";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for AGDI`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: AGDIConfig, allowFrom: string[]): AGDIConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
