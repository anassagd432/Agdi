...
import { ensureAgdiExecMarkerOnProcess } from "./infra/openclaw-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "agdi.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "agdi.js", entryBasename: "entry.js" },
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.AGDI_AUTH_STORE_READONLY = "1";
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `Agdi ${VERSION} (${commit})` : `Agdi ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[agdi] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureCliRespawnReady()) {
    ...
    if (!parsedContainer.ok) {
      console.error(`[agdi] ${parsedContainer.error}`);
      process.exit(2);
    }
    ...
    if (!parsed.ok) {
      ...
      console.error(`[agdi] ${parsed.error}`);
      process.exit(2);
    }

    const containerTargetName = resolveCliContainerTarget(process.argv);
    if (containerTargetName && parsed.profile) {
      console.error("[agdi] --container cannot be combined with --profile/--dev");
      process.exit(2);
    }
