...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "agdi-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.agdi when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "agdi-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers AGDI_HOME over HOME", () => {
    vi.stubEnv("AGDI_HOME", "/srv/agdi-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/agdi-home"));
...
describe("shortenHomePath", () => {
  it("uses $AGDI_HOME prefix when AGDI_HOME is set", () => {
    vi.stubEnv("AGDI_HOME", "/srv/agdi-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/agdi-home")}/.agdi/agdi.json`)).toBe(
      "$AGDI_HOME/.agdi/agdi.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $AGDI_HOME replacement when AGDI_HOME is set", () => {
    vi.stubEnv("AGDI_HOME", "/srv/agdi-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/agdi")).toBe(path.resolve(os.homedir(), "agdi"));
  });
  ...
  it("prefers AGDI_HOME for tilde expansion", () => {
    vi.stubEnv("AGDI_HOME", "/srv/agdi-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/agdi")).toBe(path.resolve("/srv/agdi-home", "agdi"));
