...
describe("canvas host state dir defaults", () => {
  ...
  it("uses AGDI_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("AGDI Canvas");
    } finally {
      await handler.close();
