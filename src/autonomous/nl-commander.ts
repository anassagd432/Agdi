...
export class NLCommander {
  ...
  async execute(command: string): Promise<CommandResult> {
    ...
    try {
      ...
      log.info(`plan: ${plan.steps.length} steps, risk=${plan.riskLevel}, confidence=${plan.confidence}`);
  ...
  private async planFromCommand(command: string, _screenshot: Buffer): Promise<CommandPlan> {
    ...
    if (openAppMatch) {
      ...
      return {
        ...
        steps: [{
          ...
          action: { action: "device_open_app", appName, confidence: 0.95, reasoning: `User wants to open ${appName}` },
          waitAfterMs: 2000,
        }],
        confidence: 0.95,
    ...
    if (urlMatch) {
      return {
        ...
        steps: [{
          ...
          action: { action: "device_open_url", url: urlMatch[1], confidence: 0.95, reasoning: `Navigate to URL` },
          waitAfterMs: 2000,
        }],
        confidence: 0.95,
        requiresApproval: false,
        ...
        steps: [
          {
            ...
            action: { action: "device_open_url", url: `https://www.google.com/search?q=${encodeURIComponent(query)}`, confidence: 0.9, reasoning: "Open search in browser" },
            waitAfterMs: 3000,
          },
        ],
        confidence: 0.9,
    ...
    const imageMatch = cmd.match(/^(?:click|find and click|tap)\s+(?:on\s+)?(?:the\s+)?(?:image|template|icon)\s+(.+)/i);
    if (imageMatch) {
      ...
      return {
        ...
        steps: [{
          ...
          action: { action: "device_click_image", imageTemplatePath: imgPath, confidence: 0.95, reasoning: "User wants to click an image pattern" },
          waitAfterMs: 1000,
        }],
        confidence: 0.95,
        requiresApproval: false,
    ...
    if (typeMatch) {
      return {
        ...
        steps: [{
          ...
          action: { action: "device_type", text: typeMatch[1], confidence: 0.9, reasoning: "User wants to type text" },
          waitAfterMs: 500,
        }],
        confidence: 0.9,
        requiresApproval: false,
    ...
    if (windowMatch) {
      ...
      return {
        ...
        steps: [{
          ...
          action: { action: actionMap[action!]! as any, windowTitle: target?.trim(), confidence: 0.85, reasoning: `User wants to ${action} a window` },
          waitAfterMs: 500,
        }],
        confidence: 0.85,
        requiresApproval: false,
    ...
    if (cmd.includes("screenshot") || cmd.includes("screen capture") || cmd.includes("take a picture")) {
      return {
        intent: "Take a screenshot",
        steps: [{
          description: "Capturing screen",
          action: { action: "device_screenshot", confidence: 0.95, reasoning: "User wants a screenshot" },
          waitAfterMs: 500,
        }],
        confidence: 0.95,
        requiresApproval: false,
    ...
    // --- Pattern: hotkey ---
    const hotkeyMatch = cmd.match(/^(?:press|hit|do)\s+(ctrl|alt|shift|meta|cmd|super)\s*\+\s*(.+)/i);
    if (hotkeyMatch) {
      ...
      return {
        ...
        steps: [{
          ...
          action: { action: "device_hotkey", modifiers: [mod as any], key, confidence: 0.9, reasoning: "User wants a hotkey" },
          waitAfterMs: 500,
        }],
        confidence: 0.9,
        requiresApproval: false,
    ...
    if (installMatch) {
      return {
        ...
        steps: [{
          ...
          action: { action: "device_open_app", appName: `sudo apt-get install -y ${installMatch[1]}`, confidence: 0.7, reasoning: "Install package" },
          waitAfterMs: 5000,
        }],
        confidence: 0.7,
    ...
    if (switchMatch) {
      return {
        ...
        steps: [{
          ...
          action: { action: "device_focus_window", windowTitle: switchMatch[1]!.trim(), confidence: 0.85, reasoning: "Focus window" },
          waitAfterMs: 500,
        }],
        confidence: 0.85,
        requiresApproval: false,
    ...
    if (pentestMatch) {
      ...
      setTimeout(async () => {
        ...
        try {
           ...
           log.info(`AutoPentest Complete! Found ${report.vulnerabilities.length} vulns and ${report.exploitsFound.length} exploit chains.`);
      ...
      return {
        ...
        steps: [{
          ...
          action: { action: "exec", command: `echo "Starting Pentest Playbook for ${target}"`, confidence: 0.99, reasoning: "Booting AutoPentester" } as any,
          waitAfterMs: 1000,
        }],
        confidence: 0.99,
        requiresApproval: true,
