...
export class AutoPentester {
  ...
  private emit(stage: PlaybookEvent["stage"], status: PlaybookEvent["status"], message: string, data?: any) {
    if (this.onEvent) {
  ...
  async runFullPlaybook(): Promise<PentestReport> {
    ...
    try {
      ...
      try {
          ...
          this.emit("recon", "running", "Nmap binary missing, falling back to simulated scan matrix...");
...
22/tcp   open  ssh     OpenSSH 8.2p1 Ubuntu 4ubuntu0.11`;
      ...
      if (webPorts.length > 0) {
        this.emit("vuln-scan", "started", `Found web services on ports ${webPorts.map(p => p.port).join(",")}. Initiating Nikto...`);
        for (const wp of webPorts) {
            ...
            try {
                ...
                if (!niktoResult.stdout || niktoResult.stdout.trim() === "") throw new Error("Nikto missing");
            } catch (err) {
...
[INFO] GET parameter 'id' appears to be 'MySQL >= 5.0.12 AND time-based blind' injectable`;
            ...
            report.vulnerabilities.push({ tool: "nikto", port: wp.port, output: this.truncate(niktoResult.stdout, 1000) });
            ...
            if (niktoResult.stdout.includes("php?id=") || niktoResult.stdout.includes("asp?id=")) {
                this.emit("web-attack", "running", `Potential injection point found. Firing SQLmap at ${niktoUrl}...`);
                try {
                   sqlResult = await WebAttacks.sqlmap(niktoUrl, "--batch", "--random-agent");
                } catch(e) {} // Fallback to sqlResult.stdout above
                report.vulnerabilities.push({ tool: "sqlmap", port: wp.port, output: this.truncate(sqlResult.stdout, 1000) });
            }
        }
      ...
      for (const service of parsedPorts) {
        if (service.version && service.version.length > 3) {
            ...
            const searchQuery = service.version.replace(/^(httpd|ssh|ftp|smtp)\s/i, "").trim().split(" ")[0]; // Grab main version chunk
            
            if (searchQuery.length > 2) {
                let searchResult: any = { exitCode: 0, stdout: "" };
                try {
                    searchResult = await Exploitation.searchsploit(searchQuery);
                    if (!searchResult.stdout || searchResult.stdout.trim() === "") throw new Error("Searchsploit missing");
                } catch(e) {
...
OpenSSH 8.2p1 - Pre-Auth Auth Bypass       | exploits/linux/remote/34211.txt`;
                ...
                if (searchResult.stdout.length > 20) {
                     report.exploitsFound.push({ service: service.service, version: service.version, hits: this.truncate(searchResult.stdout, 800) });
                }
            }
        }
      }
      this.emit("exploitation", "completed", `Found ${report.exploitsFound.length} potential exploit chains.`);
  ...
  private parseNmapPorts(nmapOutput: string): Array<{port: number, service: string, version: string}> {
