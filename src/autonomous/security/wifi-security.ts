...
export class WifiSecurity {
  ...
  on(l: WifiListener): void {
    this.listeners.push(l);
  }
  private emit(e: WifiEvent): void {
  ...
  async disableMonitor(iface?: string): Promise<void> {
    ...
    if (!t) return;
    try {
      await run("airmon-ng", ["stop", t], { timeout: 10000 });
    } catch {
  ...
  async scanNetworks(): Promise<WifiNetwork[]> {
    ...
    try {
      ...
      for (const line of stdout.split("\n")) {
        ...
        const bssid = p.slice(0, 6).join(":");
        const rest = p.slice(6);
        const net: WifiNetwork = {
          bssid,
          ssid: rest[0] || "(hidden)",
          channel: parseInt(rest[1] ?? "0"),
          signal: parseInt(rest[2] ?? "0"),
          encryption: this.enc(rest[3] ?? ""),
          hidden: !rest[0],
          wps: rest[3]?.includes("WPS"),
        };
        networks.push(net);
        this.emit({ kind: "network_found", network: net });
      }
    } catch (e) {
      this.emit({ kind: "error", message: `Scan failed: ${e}` });
    }
    return networks;
  }
  ...
  async captureHandshake(
    ...
    try {
      ...
      if (/1 handshake/i.test(stdout)) {
        ...
        this.emit({ kind: "handshake_captured", result: r });
        return r;
      }
    } catch {}
  ...
  async deauth(opts: DeauthOptions): Promise<void> {
    ...
    try {
      ...
      this.emit({ kind: "error", message: `Deauth failed: ${e}` });
    }
  }
  ...
  async testWps(bssid: string, iface: string): Promise<{ vulnerable: boolean; pin?: string }> {
    try {
      const { stdout } = await run(
        "timeout",
        ["30", "reaver", "-i", iface, "-b", bssid, "-vvv", "-K", "1"],
        { timeout: 35000 },
  ...
  async crackHandshake(
    capFile: string,
    wordlist = "/usr/share/wordlists/rockyou.txt",
  ): Promise<{ cracked: boolean; password?: string }> {
    try {
