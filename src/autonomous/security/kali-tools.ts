...
export interface ToolExecResult { tool: string; exitCode: number; stdout: string; stderr: string; durationMs: number; }

/** Execute any Kali tool directly */
async function exec(tool: string, args: string[], timeoutMs = 120_000): Promise<ToolExecResult> {
  const start = Date.now();
  try {
    const { stdout, stderr } = await run(tool, args, { timeout: timeoutMs, maxBuffer: 50 * 1024 * 1024 });
    return { tool, exitCode: 0, stdout, stderr, durationMs: Date.now() - start };
  } catch (err: unknown) {
    const e = err as { stdout?: string; stderr?: string; code?: number };
    return { tool, exitCode: e.code ?? 1, stdout: e.stdout ?? "", stderr: e.stderr ?? String(err), durationMs: Date.now() - start };
  }
}
...
export const InfoGathering = {
  ...
  masscan: (target: string, ports: string, rate = "1000") => exec("masscan", [target, "-p", ports, "--rate", rate]),
  amass: (domain: string) => exec("amass", ["enum", "-passive", "-d", domain], 300_000),
  sublister: (domain: string) => exec("sublist3r", ["-d", domain]),
  theHarvester: (domain: string, source = "all") => exec("theHarvester", ["-d", domain, "-b", source]),
  ...
  snmpwalk: (target: string, community = "public") => exec("snmpwalk", ["-v2c", "-c", community, target]),
...
export const VulnAnalysis = {
  ...
  openvas: (target: string) => exec("gvm-cli", ["socket", "--socketpath", "/var/run/gvmd/gvmd.sock", "--xml", `<create_target><name>${target}</name><hosts>${target}</hosts></create_target>`]),
...
export const WebAttacks = {
  sqlmap: (url: string, ...args: string[]) => exec("sqlmap", ["-u", url, "--batch", ...args], 300_000),
  burpsuite: () => exec("burpsuite", []),
  zaproxy: (target: string) => exec("zap-cli", ["quick-scan", target]),
  dirb: (url: string, wordlist = "/usr/share/wordlists/dirb/common.txt") => exec("dirb", [url, wordlist]),
  gobuster: (url: string, wordlist = "/usr/share/wordlists/dirb/common.txt") => exec("gobuster", ["dir", "-u", url, "-w", wordlist]),
  ffuf: (url: string, wordlist: string) => exec("ffuf", ["-u", `${url}/FUZZ`, "-w", wordlist]),
  wfuzz: (url: string, wordlist: string) => exec("wfuzz", ["-c", "-w", wordlist, "--hc", "404", `${url}/FUZZ`]),
...
export const PasswordAttacks = {
  hydra: (target: string, service: string, userList: string, passlist: string) => exec("hydra", ["-L", userList, "-P", passlist, target, service], 600_000),
  hashcat: (hash: string, mode: string, wordlist: string) => exec("hashcat", ["-m", mode, "-a", "0", hash, wordlist, "--force"], 600_000),
  john: (hashFile: string, wordlist = "/usr/share/wordlists/rockyou.txt") => exec("john", [`--wordlist=${wordlist}`, hashFile], 600_000),
  johnShow: (hashFile: string) => exec("john", ["--show", hashFile]),
  medusa: (target: string, module: string, userList: string, passList: string) => exec("medusa", ["-h", target, "-U", userList, "-P", passList, "-M", module]),
...
export const WirelessAttacks = {
  ...
  aircrackNg: (capFile: string, wordlist: string) => exec("aircrack-ng", ["-w", wordlist, capFile], 600_000),
  reaver: (iface: string, bssid: string) => exec("reaver", ["-i", iface, "-b", bssid, "-vvv"]),
  pixiewps: (pkE: string, pkR: string, eHash1: string, eHash2: string, authKey: string) => exec("pixiewps", ["-e", pkE, "-r", pkR, "-s", eHash1, "-z", eHash2, "-a", authKey]),
...
export const Exploitation = {
  ...
  crackmapexec: (proto: string, target: string, ...args: string[]) => exec("crackmapexec", [proto, target, ...args]),
...
export const SniffSpoof = {
  ...
  tcpdump: (iface: string, filter = "") => exec("tcpdump", ["-i", iface, ...(filter ? [filter] : [])]),
  tshark: (iface: string, ...args: string[]) => exec("tshark", ["-i", iface, ...args]),
  ettercap: (iface: string) => exec("ettercap", ["-T", "-i", iface]),
  arpspoof: (iface: string, target: string, gateway: string) => exec("arpspoof", ["-i", iface, "-t", target, gateway]),
...
export const PostExploitation = {
  linpeas: () => exec("bash", ["-c", "curl -fsSL https://github.com/carlospolop/PEASS-ng/releases/latest/download/linpeas.sh | sh"]),
  ...
  powersploit: (module: string) => exec("powershell", ["-c", `Import-Module PowerSploit; ${module}`]),
  pivoting: (lhost: string, lport: string, rhost: string) => exec("chisel", ["server", "-p", lport, "--reverse"]),
...
export const Forensics = {
  ...
  volatility: (dumpFile: string, profile: string, plugin: string) => exec("vol.py", ["-f", dumpFile, `--profile=${profile}`, plugin]),
  ...
  bulk_extractor: (file: string, outputDir: string) => exec("bulk_extractor", ["-o", outputDir, file]),
  exiftool: (file: string) => exec("exiftool", [file]),
  steghide: (file: string, passphrase = "") => exec("steghide", ["extract", "-sf", file, "-p", passphrase]),
  strings: (file: string) => exec("strings", [file]),
  hexdump: (file: string) => exec("xxd", [file]),
  dd: (input: string, output: string, bs = "512", count = "1") => exec("dd", [`if=${input}`, `of=${output}`, `bs=${bs}`, `count=${count}`]),
};
...
export const ReverseEngineering = {
  ...
  gdb: (file: string, ...cmds: string[]) => exec("gdb", ["-batch", ...cmds.flatMap(c => ["-ex", c]), file]),
...
export async function auditKaliTools(): Promise<{ installed: string[]; missing: string[]; total: number }> {
  const allTools = [
    "nmap", "masscan", "amass", "sublist3r", "theHarvester", "dnsrecon", "dnsenum", "fierce",
    "shodan", "recon-ng", "whois", "dmitry", "spiderfoot", "enum4linux", "smbclient", "snmpwalk",
    "netdiscover", "arp-scan", "wafw00f", "nikto", "wpscan", "joomscan", "droopescan", "lynis",
    "sqlmap", "zap-cli", "dirb", "gobuster", "ffuf", "wfuzz", "commix", "xsser", "whatweb",
    "skipfish", "sslyze", "testssl.sh", "hydra", "hashcat", "john", "medusa", "ncrack",
    "cewl", "crunch", "patator", "airmon-ng", "airodump-ng", "aireplay-ng", "aircrack-ng",
    "reaver", "pixiewps", "wifite", "bettercap", "kismet", "mdk3", "msfconsole", "msfvenom",
    "searchsploit", "crackmapexec", "wireshark", "tcpdump", "tshark", "ettercap", "arpspoof",
    "dsniff", "mitmproxy", "responder", "macchanger", "sslstrip", "scapy", "volatility",
    "binwalk", "foremost", "exiftool", "steghide", "strings", "xxd", "ghidra", "r2",
    "gdb", "objdump", "strace", "ltrace", "apktool", "jadx", "gophish", "evilginx2",
    "lazagne", "bloodhound", "proxychains4", "weevely", "chisel",
  ];
  ...
  for (const tool of allTools) {
    try { await run("which", [tool]); installed.push(tool); } catch { missing.push(tool); }
  }
