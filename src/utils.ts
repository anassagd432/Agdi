...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".agdi");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.AGDI_HOME?.trim();
  if (explicitHome) {
...
// Configuration root; can be overridden via AGDI_STATE_DIR.
