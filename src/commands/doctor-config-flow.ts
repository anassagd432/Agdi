...
function stripUnknownConfigKeys(config: AGDIConfig): {
  config: AGDIConfig;
  removed: string[];
} {
  const parsed = AGDISchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: AGDIConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: AGDIConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: AGDIConfig): Promise<{
  config: AGDIConfig;
  changes: string[];
}> {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".agdi");
  const targetPath = path.join(targetDir, "agdi.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("agdi doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("agdi doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "agdi doctor --fix" to remove these keys.');
    }
  }
