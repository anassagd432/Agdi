...
describe("resolveGatewayInstallToken", () => {
  ...
  it("returns unavailable reason when token and password are both configured and mode is unset", async () => {
    ...
    expect(result.unavailableReason).toContain("agdi config set gateway.auth.mode token");
    expect(result.unavailableReason).toContain("agdi config set gateway.auth.mode password");
