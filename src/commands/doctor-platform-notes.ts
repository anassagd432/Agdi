...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".agdi", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: AGDIConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "AGDI_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(([key, value]) => key.startsWith("AGDI_") && value?.trim())
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  }
  ...
  const lines = [
    ...
    "- Use AGDI_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> AGDI_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
