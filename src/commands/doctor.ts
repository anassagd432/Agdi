...
function resolveMode(cfg: AGDIConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveAGDIPackageRoot({
  ...
  let cfg: AGDIConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("agdi configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("agdi config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("agdi setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("agdi doctor --fix")}" to apply changes.`);
  }
