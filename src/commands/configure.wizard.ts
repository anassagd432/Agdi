...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.AGDI_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.AGDI_GATEWAY_PASSWORD;
...
export async function runConfigureWizard(
  ...
  try {
    ...
    const baseConfig: AGDIConfig = snapshot.valid ? snapshot.config : {};
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.AGDI_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.AGDI_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.AGDI_GATEWAY_TOKEN;
