 ...
 * Runs as part of `agdi doctor` — config-only, no network calls.
 */
export async function noteMemorySearchHealth(cfg: AGDIConfig): Promise<void> {
  ...
  if (resolved.provider !== "auto") {
    ...
    note(
      [
        ...
        `- Add credentials: ${formatCliCommand(`agdi auth add --provider ${resolved.provider}`)}`,
  ...
  note(
    [
      ...
      `- Add credentials: ${formatCliCommand("agdi auth add --provider openai")}`,
