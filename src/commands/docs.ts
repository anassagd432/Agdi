...
const SEARCH_TOOL = "https://docs.agdi.ai/mcp.SearchAGDI";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.agdi.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('agdi docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.agdi.ai/");
      runtime.log(`Search: ${formatCliCommand('agdi docs "your query"')}`);
    }
    return;
  }
