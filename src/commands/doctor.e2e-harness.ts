...
export const resolveAGDIPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadAGDIPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveAGDIPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.AGDI_STATE_DIR;
  originalUpdateInProgress = process.env.AGDI_UPDATE_IN_PROGRESS;
  process.env.AGDI_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.AGDI_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.AGDI_UPDATE_IN_PROGRESS;
  } else {
    process.env.AGDI_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
