...
export function listAgentEntries(cfg: AGDIConfig): AgentEntry[] {
...
function resolveAgentName(cfg: AGDIConfig, agentId: string) {
...
function resolveAgentModel(cfg: AGDIConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AGDIConfig): AgentSummary[] {
