...
function pruneAuthProfiles(
  cfg: AGDIConfig,
  profileIds: Set<string>,
): { next: AGDIConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("agdi models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("agdi models auth setup-token")} or ${formatCliCommand(
      "agdi configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "agdi models auth login --provider openai-codex",
    )} or ${formatCliCommand("agdi configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("agdi configure")}\` or \`${formatCliCommand("agdi onboard")}\`.`;
}
