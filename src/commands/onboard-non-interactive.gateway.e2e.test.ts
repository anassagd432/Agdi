...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.AGDI_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.AGDI_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "agdi");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.AGDI_STATE_DIR = stateDir;
    process.env.AGDI_CONFIG_PATH = path.join(stateDir, "agdi.json");
