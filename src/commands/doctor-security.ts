...
import type { AGDIConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AGDIConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("agdi security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("agdi configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("agdi config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("agdi doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("agdi config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
