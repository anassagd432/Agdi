...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAGDIAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("agdi models auth setup-token")}\` or \`${formatCliCommand("agdi configure")}\`.`
          : `Run \`${formatCliCommand("agdi configure")}\` or set an API key env var.`;
