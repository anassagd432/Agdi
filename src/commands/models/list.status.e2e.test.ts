...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAGDIAgentDir: vi.fn().mockReturnValue("/tmp/agdi-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/agdi-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/agdi-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAGDIAgentDir: mocks.resolveAGDIAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAGDIAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/agdi-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/agdi-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/agdi-agent-custom");
