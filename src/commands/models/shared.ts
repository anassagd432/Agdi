...
export function resolveModelTarget(params: { raw: string; cfg: AGDIConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AGDIConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("agdi agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
