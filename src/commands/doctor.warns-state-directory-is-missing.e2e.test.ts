...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "agdi-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.AGDI_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when AGDI_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/agdi.json",
      exists: true,
      raw: "{}",
    ...
    process.env.AGDI_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
