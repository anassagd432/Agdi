...
describe("doctor command", () => {
  ...
  it("warns when token and password are both configured and gateway.auth.mode is unset", async () => {
    ...
    expect(String(gatewayAuthNote?.[0])).toContain("agdi config set gateway.auth.mode token");
    expect(String(gatewayAuthNote?.[0])).toContain("agdi config set gateway.auth.mode password");
  });
