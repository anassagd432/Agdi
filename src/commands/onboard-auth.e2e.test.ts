...
const requireAgentDir = () => {
  const agentDir = process.env.AGDI_AGENT_DIR;
  if (!agentDir) {
    throw new Error("AGDI_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under AGDI_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-oauth-"));
    process.env.AGDI_STATE_DIR = tempStateDir;
    process.env.AGDI_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AGDI_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to AGDI_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-minimax-"));
    process.env.AGDI_STATE_DIR = tempStateDir;
    process.env.AGDI_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AGDI_AGENT_DIR;
