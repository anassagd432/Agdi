...
export async function statusCommand(
  ...
  runtime.log(theme.heading("AGDI status"));
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("agdi status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("agdi logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("agdi status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("agdi gateway probe")}`);
  }
}
