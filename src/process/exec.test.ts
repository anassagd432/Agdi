...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["AGDI_BASE_ENV"]);
    process.env.AGDI_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.AGDI_BASE_ENV ?? "") + "|" + (process.env.AGDI_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { AGDI_TEST_ENV: "ok" },
        },
      );
