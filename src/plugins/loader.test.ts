...
import { loadAGDIPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `agdi-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.AGDI_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "agdi.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.AGDI_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadAGDIPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.AGDI_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadAGDIPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadAGDIPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
