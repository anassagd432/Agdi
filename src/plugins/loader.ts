...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AGDIPluginDefinition;
  register?: AGDIPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AGDIPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AGDIPluginDefinition;
...
export function loadAGDIPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAGDIPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AGDIPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as AGDIPluginModule;
    } catch (err) {
