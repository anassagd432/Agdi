...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `agdi-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.agdi/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without agdi.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@agdi/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("agdi.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@agdi/voice-call@0.0.1", "--ignore-scripts"]);
