...
const DEFAULT_PACKAGE_NAME = "agdi";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "agdi.mjs");
    ...
    const doctorStep = await runStep(
      step("agdi doctor", doctorArgv, gitRoot, { AGDI_UPDATE_IN_PROGRESS: "1" }),
    );
