...
function candidateBinDirs(opts: EnsureAGDIPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `agdi` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "agdi");
  ...
  if (allowProjectLocalBin) {
    ...
    if (isExecutable(path.join(localBinDir, "agdi"))) {
...
export function ensureAGDICliOnPath(opts: EnsureAGDIPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AGDI_PATH_BOOTSTRAPPED)) {
    return;
  }
