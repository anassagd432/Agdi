...
let ensureAGDICliOnPath: typeof import("./path-env.js").ensureAGDICliOnPath;

describe("ensureAGDICliOnPath", () => {
  const envKeys = [
    "PATH",
    "AGDI_PATH_BOOTSTRAPPED",
    "AGDI_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureAGDICliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling agdi exists", () => {
    const tmp = abs("/tmp/agdi-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "agdi");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.AGDI_PATH_BOOTSTRAPPED;
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/agdi-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "agdi");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/agdi-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "agdi");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "agdi");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/agdi-path/case-linuxbrew");
