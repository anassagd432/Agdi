...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.AGDI_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.AGDI_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.AGDI_TEST_TAILSCALE_BINARY;
    } else {
      process.env.AGDI_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
