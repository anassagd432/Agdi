...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AGDI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*agdi-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "agdi";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "agdi");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "agdi-gw");
  ...
  records.push(`_agdi-gw._tcp IN PTR ${instanceLabel}._agdi-gw._tcp`);
  records.push(`${instanceLabel}._agdi-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._agdi-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; agdi-content-hash: ${contentHash}\n${contentBody}`;
}
