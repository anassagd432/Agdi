...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AGDI_DISABLE_BONJOUR ?? process.env.OPENCLAW_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Agdi";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\((?:Agdi|OpenClaw)\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    (process.env.AGDI_MDNS_HOSTNAME ?? process.env.OPENCLAW_MDNS_HOSTNAME)?.trim() || "agdi";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
