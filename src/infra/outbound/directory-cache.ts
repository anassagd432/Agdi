...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AGDIConfig | null = null;
  ...
  get(key: string, cfg: AGDIConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AGDIConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AGDIConfig): void {
