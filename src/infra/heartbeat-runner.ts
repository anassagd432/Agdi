...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: AGDIConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: AGDIConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: AGDIConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: AGDIConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: AGDIConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: AGDIConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: AGDIConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function resolveHeartbeatSession(cfg: AGDIConfig, agentId?: string, heartbeat?: HeartbeatConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: AGDIConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
