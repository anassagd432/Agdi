...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__agdi_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "agdi-root");
...
describe("resolveAGDIPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const pkgRoot = path.join(project, "node_modules", "agdi");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "agdi" }));

    expect(resolveAGDIPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "agdi.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "agdi" }));

    expect(resolveAGDIPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveAGDIPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });
  ...
  it("returns null for non-agdi package roots", async () => {
    ...
    expect(resolveAGDIPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveAGDIPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
