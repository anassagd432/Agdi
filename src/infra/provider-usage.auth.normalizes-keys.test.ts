...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(
    ...
    await fs.mkdir(path.join(base, ".agdi", "agents", "main", "sessions"), { recursive: true });
    ...
    delete process.env.AGDI_HOME;
    process.env.AGDI_STATE_DIR = path.join(base, ".agdi");
  ...
  it("strips embedded CR/LF from stored auth profiles (token + api_key)", async () => {
    await withSuiteHome(
      async (home) => {
        const agentDir = path.join(home, ".agdi", "agents", "main", "agent");
