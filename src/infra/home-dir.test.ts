...
describe("resolveEffectiveHomeDir", () => {
  it("prefers AGDI_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/agdi-home"));
  });
  ...
  it("expands AGDI_HOME when set to ~", () => {
    const env = {
      AGDI_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for AGDI_HOME", () => {
    const result = resolveRequiredHomeDir(
      { AGDI_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when AGDI_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ AGDI_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { AGDI_HOME: "/srv/agdi-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/agdi-home")}/x`);
  });
