...
This document describes how the Agdi ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`agdi acp` exposes an ACP agent over stdio and forwards prompts to a running
Agdi Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `agdi acp` and speaks ACP messages over stdio.
...
`agdi acp` resolves the Gateway URL and auth from CLI flags or config:
