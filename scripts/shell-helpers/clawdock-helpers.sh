#!/usr/bin/env bash
# ClawDock - Docker helpers for Agdi
# Inspired by Simon Willison's "Running Agdi in Docker"
# https://til.simonwillison.net/llms/agdi-docker
#
# Installation:
...
# Common paths to check for Agdi
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Agdi at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/agdi"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Agdi not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/agdi/agdi.git ~/agdi"
    echo "  cd ~/agdi && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^AGDI_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d agdi-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart agdi-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f agdi-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec agdi-gateway \
    bash -c 'echo "alias agdi=\"./agdi.mjs\"" > /tmp/.bashrc_agdi && bash --rcfile /tmp/.bashrc_agdi'
}
...
clawdock-cli() {
  _clawdock_compose run --rm agdi-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build agdi-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "AGDI_GATEWAY_TOKEN=$token" agdi-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" agdi-gateway \
    bash -c './agdi.mjs config set gateway.remote.token "$TOKEN" && ./agdi.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec agdi-gateway \
    bash -c "./agdi.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart agdi-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm agdi-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec agdi-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'agdi config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Agdi${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (agdi alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to agdi project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.agdi)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'agdi channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'agdi status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.agdi.ai${_CLR_RESET}"
  echo ""
}
