...
#
# Suggested cron: */30 * * * * /home/admin/agdi/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Agdi notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., agdi-alerts)
...
STATE_FILE="$HOME/.agdi/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Agdi if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use agdi
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Agdi to $NOTIFY_PHONE..."
            agdi send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Agdi is down. Run: ssh l36 '~/agdi/scripts/mobile-reauth.sh'" "urgent"
    exit 1
