...
# Check auth status
termux-toast "Checking Agdi auth..."

STATUS=$(ssh "$SERVER" '$HOME/agdi/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/agdi/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|AGDI_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/agdi/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|AGDI_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Agdi won't work until re-auth"
                ;;
        esac
        ;;
