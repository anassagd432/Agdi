#!/bin/bash
# Setup Agdi Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'agdi-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Agdi message: Send warning via Agdi itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/agdi-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/agdi-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/agdi-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now agdi-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status agdi-auth-monitor.timer"
echo ""
echo "Quick commands:"
