 ...
 * `agdi update`. Use it to verify the completion installation flow
...
\`agdi update\`, \`agdi doctor\`, and \`agdi onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Agdi Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`agdi completion --install\` later to enable.`));
    return;
  }
