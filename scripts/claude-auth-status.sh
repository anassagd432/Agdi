...
# Checks both Claude Code and Agdi auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
AGDI_AUTH="$HOME/.agdi/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    agdi models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    agdi_status=$(check_agdi_auth 2>/dev/null || true)

    claude_expires=0
    agdi_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        agdi_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        agdi_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$AGDI_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            agdi: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$agdi_status" == EXPIRED* ]] || [[ "$agdi_status" == MISSING* ]]; then
        echo "AGDI_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$agdi_status" == EXPIRING* ]]; then
        echo "AGDI_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Agdi Auth (~/.agdi/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$AGDI_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$AGDI_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'agdi doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active agdi >/dev/null 2>&1; then
    echo -e "Agdi service: ${GREEN}running${NC}"
else
    echo -e "Agdi service: ${RED}NOT running${NC}"
fi
