...
IMAGE_NAME="agdi-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="agdi-net-e2e-$$"
GW_NAME="agdi-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "AGDI_GATEWAY_TOKEN=$TOKEN" \
	  -e "AGDI_SKIP_CHANNELS=1" \
	  -e "AGDI_SKIP_GMAIL_WATCHER=1" \
	  -e "AGDI_SKIP_CRON=1" \
	  -e "AGDI_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
