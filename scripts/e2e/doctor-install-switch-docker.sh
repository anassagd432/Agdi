...
IMAGE_NAME="agdi-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/agdi-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/agdi-bin/systemctl

  cat > /tmp/agdi-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
LOGINCTL
  ...
  npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz"
	  ...
	  npm_bin="/tmp/npm-prefix/bin/agdi"
	  npm_root="/tmp/npm-prefix/lib/node_modules/agdi"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/agdi-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/agdi-gateway.service"
    if [ ! -f "$unit_path" ]; then
