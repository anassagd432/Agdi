...
# Build and bundle Agdi into a minimal .app we can open.
# Outputs to dist/Agdi.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Agdi.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Agdi"
BUNDLE_ID="${BUNDLE_ID:-ai.agdi.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/agdi/agdi/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Agdi/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AgdiBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AgdiGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Agdi"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Agdi"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Agdi"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Agdi" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Agdi/Resources/Agdi.icns" "$APP_ROOT/Contents/Resources/Agdi.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Agdi/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AgdiKit resources"
AGDIKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AgdiKit_AgdiKit.bundle"
if [ -d "$AGDIKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AgdiKit_AgdiKit.bundle"
  cp -R "$AGDIKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AgdiKit_AgdiKit.bundle"
else
  echo "WARN: AgdiKit resource bundle not found at $AGDIKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Agdi"
killall -q Agdi 2>/dev/null || true
