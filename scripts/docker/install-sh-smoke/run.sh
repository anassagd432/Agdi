...
SMOKE_PREVIOUS_VERSION="${AGDI_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${AGDI_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="agdi"
PACKAGE_NAME="${AGDI_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (AGDI_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${AGDI_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${AGDI_INSTALL_LATEST_OUT:-}"
fi
