...
MODELS_MODE="${AGDI_E2E_MODELS:-${AGDI_E2E_MODELS:-both}}" # both|openai|anthropic
INSTALL_TAG="${AGDI_INSTALL_TAG:-latest}"
E2E_PREVIOUS_VERSION="${AGDI_INSTALL_E2E_PREVIOUS:-${AGDI_INSTALL_E2E_PREVIOUS:-}}"
SKIP_PREVIOUS="${AGDI_INSTALL_E2E_SKIP_PREVIOUS:-${AGDI_INSTALL_E2E_SKIP_PREVIOUS:-0}}"
...
if [[ "$INSTALL_TAG" == "beta" ]]; then
  AGDI_BETA=1 AGDI_BETA=1 curl -fsSL "$INSTALL_URL" | bash
elif [[ "$INSTALL_TAG" != "latest" ]]; then
  AGDI_VERSION="$INSTALL_TAG" AGDI_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash
else
