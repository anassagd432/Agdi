...
const tmpHome = mkdtempSync(path.join(os.tmpdir(), "agdi-startup-memory-"));
...
const cases = [
  {
    ...
    args: ["agdi.mjs", "--help"],
    limitMb: Number(
      process.env.AGDI_STARTUP_MEMORY_HELP_MB ??
        process.env.OPENCLAW_STARTUP_MEMORY_HELP_MB ??
        DEFAULT_LIMITS_MB.help,
    ),
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["agdi.mjs", "status", "--json"],
    limitMb: Number(
      process.env.AGDI_STARTUP_MEMORY_STATUS_JSON_MB ??
        process.env.OPENCLAW_STARTUP_MEMORY_STATUS_JSON_MB ??
  ...
  },
  {
    ...
    args: ["agdi.mjs", "gateway", "status"],
    limitMb: Number(
      process.env.AGDI_STARTUP_MEMORY_GATEWAY_STATUS_MB ??
        process.env.OPENCLAW_STARTUP_MEMORY_GATEWAY_STATUS_MB ??
...
function formatFixGuidance(testCase, details) {
  ...
  const guidance = [
    ...
    "LLM prompt:",
    `"Agdi startup-memory CI failed for '${testCase.label}'. Analyze this failure, identify the first runtime/import side effect that makes startup heavier or broken, and propose the smallest safe patch. Failure output:\n${details}"`,
  ];
