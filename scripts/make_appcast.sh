...
ZIP=${1:?"Usage: $0 Agdi-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/agdi/agdi/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Agdi-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/agdi/agdi/releases/download/v${VERSION}/"}
