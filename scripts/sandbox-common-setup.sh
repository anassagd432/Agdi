...
BASE_IMAGE="${BASE_IMAGE:-agdi-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-agdi-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=agdi.sandbox=1)
NOTE
