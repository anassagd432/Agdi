...
export async function handleWhatsAppStart(host: AgdiApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: AgdiApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: AgdiApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: AgdiApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: AgdiApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: AgdiApp): string {
...
function resolveGatewayHttpAuthHeader(host: AgdiApp): string | null {
...
function buildGatewayHttpHeaders(host: AgdiApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: AgdiApp) {
...
export function handleNostrProfileToggleAdvanced(host: AgdiApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: AgdiApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: AgdiApp) {
  const state = host.nostrProfileFormState;
