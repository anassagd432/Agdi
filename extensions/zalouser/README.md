...
AGDI extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
agdi message send --channel zalouser --target <threadId> --message "Hello from AGDI!"
```
...
```bash
agdi channels login --channel zalouser              # Login via QR
...
```bash
# Text
agdi message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
agdi message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `agdi channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
agdi channels login --channel zalouser --account work
agdi message send --channel zalouser --account work --target <id> --message "Hello"
