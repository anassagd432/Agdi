...
function resolveAGDIRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.AGDI_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set AGDI_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAGDIRoot(), "dist", "extensionAPI.js");
