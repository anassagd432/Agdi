...
vi.mock("agdi/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("agdi/plugin-sdk/reply-runtime")>(
...
vi.mock("agdi/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("agdi/plugin-sdk/security-runtime")>(
...
vi.mock("agdi/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("agdi/plugin-sdk/infra-runtime")>(
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("agdi/plugin-sdk/reply-runtime"),
      import("agdi/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
