...
function normalizeDenyList(cfg: AGDIPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AGDIPluginApi["config"]): string[] {
...
export default function register(api: AGDIPluginApi) {
  ...
  const timerService: AGDIPluginService = {
