...
metadata: { "agdi": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == AGDI `sessions_spawn`
- **File I/O** == AGDI `read`/`write`
- **Remote fetch** == AGDI `web_fetch` (or `exec` with curl when POST is required)
