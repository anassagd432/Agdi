...
import type { AGDIPluginApi, AGDIPluginToolContext } from "../../../src/plugins/types.js";
...
function fakeApi(overrides: Partial<AGDIPluginApi> = {}): AGDIPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<AGDIPluginToolContext> = {}): AGDIPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "agdi-lobster-plugin-"));
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: AGDIPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
