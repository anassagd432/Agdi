...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AGDIConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AGDIConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as AGDIConfig);
