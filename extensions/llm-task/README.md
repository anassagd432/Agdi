...
`agdi.invoke --each`) without adding new AGDI code per workflow.
...
This extension depends on AGDI internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AGDI extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.agdi/extensions` as a standalone plugin directory.
